...
# Test 2: test_remember_memory_user_id_alignment
...
if [[ $remember_has_project_user_id -gt 0 && $memory_has_project_user_id -gt 0 ]]; then
  ...
  fail_test "User ID mismatch" "remember: $remember_has_project_user_id occurrences, memory: $memory_has_project_user_id occurrences"
fi
echo ""
...
if [[ $remember_has_graph -gt 0 && $memory_has_graph -gt 0 ]]; then
  ...
  fail_test "Graph flag incomplete" "remember: $remember_has_graph occurrences, memory: $memory_has_graph occurrences"
fi
echo ""
...
if [[ $remember_has_global_user_id -gt 0 && $memory_has_global_user_id -gt 0 ]]; then
  ...
  fail_test "Global user_id missing" "remember: $remember_has_global_user_id occurrences, memory: $memory_has_global_user_id occurrences"
fi
echo ""
...
if [[ $remember_has_add_memory -gt 0 && $memory_has_search_memories -gt 0 ]]; then
  ...
  fail_test "mem0 script integration incomplete" "remember (add): $remember_has_add_memory, memory (search): $memory_has_search_memories"
fi
echo ""
