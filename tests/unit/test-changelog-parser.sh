...
# The payload had no account field, so the platform picked the handle by which
# 1Password credential set it loaded — and the unprefixed Twitter-* items are
# the operator's PERSONAL @yonyoniz. An automated rail must never draft as him.
...
if [[ "$acct" == "yonyon2ai" ]]; then
  pass "announce payload defaults to the machine account (@yonyon2ai)"
else
