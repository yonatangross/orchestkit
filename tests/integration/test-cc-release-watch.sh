...
# version" scenario (2.1.206 — the synthetic fixture's top version, the only
...
# cc-support.json.latest (currently 2.1.206) so Test 4's recovery target — which
...
if [ -f shared/cc-snapshots/2.1.206.md ]; then
  log_pass "Snapshot written for new version 2.1.206"
else
  log_fail "Snapshot missing" "expected shared/cc-snapshots/2.1.206.md (see /tmp/watch-out.txt)"
fi

if [ -f shared/cc-snapshots/2.1.206.md ] && grep -qF "EnterWorktree" shared/cc-snapshots/2.1.206.md; then
...
if [ -f shared/cc-adoption-gaps.json ]; then
  ...
  if [ "$COUNT" = "1" ]; then
    ...
    if [ "$GAP_VERSION" = "2.1.206" ]; then
      log_pass "Gap entry version = 2.1.206"
    else
      log_fail "Gap entry version" "expected 2.1.206, got '$GAP_VERSION'"
    fi
  else
...
if [ -f shared/gh-issue-args.json ]; then
  ...
  if [ "$LATEST" = "2.1.206" ]; then
    log_pass "gh-issue-args latest_new_version = 2.1.206 (per-version trace kept)"
  else
    log_fail "latest_new_version" "expected 2.1.206, got '$LATEST'"
  fi
else
...
# After Test 1, the 2.1.206 gap entry is un-triaged (empty features) and
...
if [ -f shared/cc-adoption-gaps.json ]; then
  KEPT=$(jq -r '[.[] | select(.version=="2.1.206" and (has("issues_filed_at")|not))] | length' shared/cc-adoption-gaps.json)
  if [ "$KEPT" = "1" ]; then
    log_pass "No-op run preserves un-filed (un-stamped) gap entry (carry-forward, #2084 fix)"
  else
    log_fail "Idempotent gaps carry" "expected un-stamped 2.1.206 preserved on no-op run, got $KEPT"
  fi
fi
...
# (M130 hotfix). Reads cc-support.json.latest (2.1.206 today) so this test
...
rm -rf shared/cc-snapshots/*.md shared/cc-adoption-gaps.json shared/gh-issue-args.json
mkdir -p shared/cc-snapshots
for v in 2.1.126 2.1.127 2.1.128 2.1.132 2.1.138 2.1.206; do
...
NEWN=$(jq -r '[.[] | select(.version=="2.1.206")] | length' shared/cc-adoption-gaps.json)
CARRIED2=$(jq -r '[.[] | select(.version=="2.1.901" and .parse_failed==true)] | length' shared/cc-adoption-gaps.json)
if [ "$NEWN" = "1" ] && [ "$CARRIED2" = "1" ]; then
  log_pass "carry-forward (new-version): stuck entry rides alongside new 2.1.206"
else
  log_fail "carry-forward (new-version)" "expected new 2.1.206 + carried 2.1.901, got new=$NEWN carried=$CARRIED2 (see /tmp/watch-carry2.txt)"
fi
rm -rf shared/cc-snapshots/*.md shared/cc-adoption-gaps.json shared/gh-issue-args.json
