...
test_all_mcp_hooks_exist() {
    local expected_hooks=("context7-tracker.sh" "playwright-safety.sh" "memory-validator.sh" "sequential-thinking-auto.sh")
...
test_all_mcp_hooks_executable() {
    local hooks=("context7-tracker.sh" "playwright-safety.sh" "memory-validator.sh" "sequential-thinking-auto.sh")
...
test_playwright_returns_valid_json() {
    local hook="$MCP_HOOKS_DIR/playwright-safety.sh"
    [[ ! -f "$hook" ]] && skip "playwright-safety.sh not found"

    local input='{"tool_name":"mcp__playwright__navigate","tool_input":{"url":"https://example.com"}}'
    local result=$(echo "$input" | bash "$hook" 2>/dev/null)
...
test_hooks_dont_interfere_with_each_other() {
    # Test that each hook only processes its own MCP tools
    local context7_input='{"tool_name":"mcp__playwright__navigate","tool_input":{}}'
    local playwright_input='{"tool_name":"mcp__context7__query-docs","tool_input":{}}'

    # Context7 hook should pass through playwright tool
    ...
    # Playwright hook should pass through context7 tool
    if [[ -f "$MCP_HOOKS_DIR/playwright-safety.sh" ]]; then
        local pw_result=$(echo "$playwright_input" | bash "$MCP_HOOKS_DIR/playwright-safety.sh" 2>/dev/null)
        [[ "$pw_result" == *'"continue": true'* ]] || return 1
    fi
