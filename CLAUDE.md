...
**OrchestKit** — Claude Code plugin: **71 skills**, **30 agents**, **81 hooks** (30 global + 44 agent-scoped + 7 skill-scoped).
...
**Single plugin**: `ork` — all 71 skills, 30 agents, 81 hooks.

The plugin includes 81 hooks (30 global [10 matchers, 17 dispatchers, 3 native HTTP] + 44 agent-scoped + 7 skill-scoped) and a `settings.json` (permissions, keybindings, spinner). 17 skills are user-invocable via `/ork:skillname`.
...
- **Hooks**: 81 total (30 global [10 matchers, 17 dispatchers, 3 native HTTP] + 44 agent-scoped + 7 skill-scoped)
