...
Exception, the hook bundles (#3578): `src/hooks/dist/` and `plugins/ork/hooks/dist/` are release-owned. Build them locally for tests, never commit them from a feature branch; the release-please branch rebuilds them and CI fails a PR that changes them. If they show up staged, `git restore --staged src/hooks/dist plugins/ork/hooks/dist`.
...
- **Current**: 10.0.0-alpha.69 · **Claude Code**: >= 2.1.251 <!-- x-release-please-version -->
