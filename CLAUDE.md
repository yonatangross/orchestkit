...
**OrchestKit** — Claude Code plugin: **71 skills**, **30 agents**, **103 hooks** (39 global + 44 agent-scoped + 20 skill-scoped).
...
**Single plugin**: `ork` — all 71 skills, 30 agents, 103 hooks.

The plugin includes 103 hooks (39 global [10 matchers, 20 dispatchers, 4 native HTTP, 5 type:prompt] + 44 agent-scoped + 20 skill-scoped) and a `settings.json` (permissions, keybindings, spinner). 17 skills are user-invocable via `/ork:skillname`.
...
- **Current**: 7.1.3 · **Claude Code**: >= 2.1.59 <!-- x-release-please-version -->
- **Hooks**: 103 total (39 global [10 matchers, 20 dispatchers, 4 native HTTP, 5 type:prompt] + 44 agent-scoped + 20 skill-scoped)
