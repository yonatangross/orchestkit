...
- **120 hooks**: TypeScript lifecycle automation (92 global + 22 agent-scoped + 6 skill-scoped, 6 fire-and-forget dispatchers)
...
### Hooks
121 hook entries (92 global + 22 agent-scoped + 6 skill-scoped) across 11 split bundles. Auto-loaded from `hooks/hooks.json`. Return `{"continue": true}` to proceed, `{"continue": false}` to block.
...
Both include all 36 agents, 120 hooks, and all memory skills (remember, memory, memory-fabric, mem0-memory).
...
- **Current**: 6.0.2
- **Claude Code**: >= 2.1.33
- **Hooks**: 120 entries (92 global + 22 agent-scoped + 6 skill-scoped, 11 split bundles, 6 fire-and-forget dispatchers)
