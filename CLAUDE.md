...
**OrchestKit** — Claude Code plugin: **69 skills**, **38 agents**, **94 hooks** (33 global + 54 agent-scoped + 7 skill-scoped).
...
**Single plugin**: `ork` — all 69 skills, 38 agents, 94 hooks.

The plugin includes 94 hooks (33 global [12 matchers, 16 dispatchers, 7 native async] + 54 agent-scoped + 7 skill-scoped) and a `settings.json` (permissions, keybindings, spinner). 17 skills are user-invocable via `/ork:skillname`.
...
- **Hooks**: 94 total (33 global [12 matchers, 16 dispatchers, 7 native async] + 54 agent-scoped + 7 skill-scoped)
