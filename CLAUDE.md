...
**OrchestKit** — Claude Code plugin: **69 skills**, **38 agents**, **86 hooks** (36 global + 46 agent-scoped + 4 skill-scoped).
...
**Single plugin**: `ork` — all 69 skills, 38 agents, 86 hooks.

The plugin includes 86 hooks (36 global [32 command + 4 HTTP] + 46 agent-scoped + 4 skill-scoped) and a `settings.json` (permissions, keybindings, spinner). 17 skills are user-invocable via `/ork:skillname`.
...
- **Hooks**: 87 total (36 global [18 event types, 20 dispatchers, 7 native async, 4 HTTP] + 46 agent-scoped + 4 skill-scoped)
