...
const required = ['schema_version', 'namespace', 'producer', 'seam', 'mode', 'decision_id', 'phase',
    ...
    'agree', 'floor', 'decided_by', 'unknown_reason'];
...
export const eventSha256 = (record) => sha256(JSON.stringify(canonical(record)));
...
function outcome(value) {
    return !!value && typeof value === 'object' && !Array.isArray(value)
        ...
        && ['failed', 'unobserved'].includes(value.status)
...
export function validateJevShadow(record) {
    if (!record || typeof record !== 'object' || Array.isArray(record))
        return ['record:not_object'];
    const row = record, errors = [];
    ...
    if (row.schema_version !== 1)
        errors.push('schema_version:not_v1');
    if (!nonEmpty(row.namespace) || !nonEmpty(row.producer) || !nonEmpty(row.seam) || !nonEmpty(row.decision_id))
        errors.push('identity:invalid');
    if (row.mode !== 'shadow')
        errors.push('mode:not_shadow');
    if (typeof row.harness !== 'string' || !harnesses.has(row.harness))
        errors.push('harness:unknown');
    if (typeof row.phase !== 'string' || !phases.has(row.phase))
        errors.push('phase:invalid');
    if (typeof row.decided_by !== 'string' || !decidedBy.has(row.decided_by))
        errors.push('decided_by:invalid');
    if (row.unknown_reason !== null && !reasonMap(row.unknown_reason))
        errors.push('unknown_reason:invalid');
    for (const key of nullableWithReasons) {
        const value = row[key];
        if (value === null) {
            if (!reasonMap(row.unknown_reason) || !nonEmpty(row.unknown_reason[key]))
                errors.push(`unknown_reason:missing:${key}`);
        }
        else if ((key === 'jev_confidence' || key === 'floor') ? !probability(value) : !nonEmpty(value))
            errors.push(`${key}:invalid`);
    }
    if (row.incumbent_pick !== null && !nonEmpty(row.incumbent_pick) && !outcome(row.incumbent_pick))
        errors.push('incumbent_pick:invalid');
    if (typeof row.agree !== 'boolean' && row.agree !== null)
        errors.push('agree:invalid');
    if (outcome(row.incumbent_pick) && row.agree !== null)
        errors.push('agree:outcome_requires_null');
    if (row.incumbent_pick === null && (!nonEmpty(row.incumbent_pick_reason)
        || !reasonMap(row.unknown_reason) || !nonEmpty(row.unknown_reason.incumbent_pick)))
        errors.push('incumbent_pick_reason:missing');
    if (row.jev_pick === null && row.agree !== null)
        errors.push('agree:missing_jev_requires_null');
    if (typeof row.agree === 'boolean' && (!nonEmpty(row.jev_pick) || !nonEmpty(row.incumbent_pick)
        || row.incumbent_origin !== 'independent'))
        errors.push('agree:requires_independent_choices');
    if (typeof row.agree === 'boolean' && nonEmpty(row.jev_pick) && nonEmpty(row.incumbent_pick)
        && row.agree !== (row.jev_pick === row.incumbent_pick))
        errors.push('agree:inconsistent_choices');
    for (const key of optionalNullableStrings)
        if (row[key] !== undefined && row[key] !== null && !nonEmpty(row[key]))
            errors.push(`${key}:invalid`);
    if (row.selected_by !== undefined && (typeof row.selected_by !== 'string' || !selectedBy.has(row.selected_by)))
        errors.push('selected_by:invalid');
    if (row.incumbent_origin !== undefined && (typeof row.incumbent_origin !== 'string' || !origins.has(row.incumbent_origin)))
        errors.push('incumbent_origin:invalid');
    if (row.incumbent_origin === 'not_run' && row.incumbent_pick !== null)
        errors.push('incumbent_origin:not_run_requires_null');
    return errors;
}
...
async function safeDirectory(path) {
    if (!isAbsolute(path))
        throw new Error('JEV_SHADOW_ROOT must be an absolute path');
    ...
    for (const part of absolute.slice(parsed.root.length).split(sep).filter(Boolean)) {
        ...
        try {
            ...
            if (stat.isSymbolicLink() || !stat.isDirectory())
                throw new Error(`Unsafe JEV_SHADOW_ROOT component: ${current}`);
        }
        catch (error) {
    ...
    if (resolved !== absolute)
        throw new Error('JEV_SHADOW_ROOT resolves through a symlink');
    const finalStat = await lstat(absolute);
    if ((finalStat.mode & 0o077) !== 0)
        throw new Error(`Insecure JEV_SHADOW_ROOT permissions: ${absolute}`);
    return absolute;
}
async function journalPath(root, row) {
    ...
    try {
        ...
        if (stat.isSymbolicLink() || !stat.isFile() || (stat.mode & 0o077) !== 0)
            throw new Error(`Unsafe journal: ${journal}`);
    }
    catch (error) {
        if (error.code !== 'ENOENT')
            throw error;
    ...
    for (let attempt = 0; attempt < lockRetries; attempt++) {
        try {
            ...
            return async () => {
                let current;
                try {
                    current = await readFile(lock, 'utf8');
                }
                catch {
                    return;
                }
                if (current !== token)
                    throw new Error('Jev shadow lock ownership changed; refusing to remove it');
            ...
            if (stat.isSymbolicLink() || !stat.isFile() || (stat.mode & 0o077) !== 0)
                throw new Error(`Unsafe journal lock: ${lock}`);
 ...
 */
export async function emitJevShadow(record, root = process.env.JEV_SHADOW_ROOT) {
    if (!root)
        return { enabled: false, appended: false, duplicate: false, path: null };
    ...
    if (errors.length)
        throw new Error(`Invalid Jev shadow record: ${errors.join(',')}`);
...
export async function replayJevShadow(path) {
...
export function createPassiveRecord(input) {
    ...
    if (session === null)
        unknown_reason.session_id = 'session_id_unobserved';
    return {
