 ...
 */
export function firstRunSetup(_input: HookInput, hookCtx?: HookContext): HookResult {
  const pluginRoot = hookCtx?.pluginRoot ?? getPluginRoot();
  const mode = process.argv.includes('--silent') ? 'silent' : 'interactive';

  (hookCtx?.log ?? logHook)('first-run-setup', `First-run setup starting (mode: ${mode})`);
  ...
  if (!valid) {
    (hookCtx?.log ?? logHook)('first-run-setup', 'ERROR: Dependency validation failed');
  ...
  if (mode === 'interactive') {
    (hookCtx?.log ?? logHook)('first-run-setup', 'Interactive mode - using complete preset (wizard via Claude conversation)');
  } else {
    (hookCtx?.log ?? logHook)('first-run-setup', 'Silent mode - using complete preset');
  }
  ...
  (hookCtx?.log ?? logHook)('first-run-setup', `Setup complete: ${skillCount} skills, ${agentCount} agents, ${hookCount} hooks`);
