 ...
 */
export function taskCompletionCheck(input: HookInput, ctx?: HookContext): HookResult {
  (ctx?.log ?? logHook)('task-completion-check', 'Stop hook - checking task completion');
  ...
  if (existsSync(registryFile)) {
    try {
      ...
      if (inProgress.length > 0) {
        (ctx?.log ?? logHook)('task-completion-check', `WARNING: ${inProgress.length} orchestration tasks still in progress`);
        warnings.push(`${inProgress.length} orchestration task(s) still in progress at session stop`);
      }
    } catch (error) {
      (ctx?.log ?? logHook)('task-completion-check', `Error reading registry: ${error}`);
    }
  }
  ...
  if (orphans.length > 0) {
    (ctx?.log ?? logHook)('task-completion-check', `Found ${orphans.length} orphaned tasks`);
  ...
  if (existsSync(todosFile)) {
    try {
      ...
      if (inProgress.length > 0) {
        (ctx?.log ?? logHook)('task-completion-check', `WARNING: ${inProgress.length} legacy tasks in progress at stop`);
        warnings.push(`${inProgress.length} legacy task(s) still in progress`);
      }
    } catch (error) {
      (ctx?.log ?? logHook)('task-completion-check', `Error reading legacy todos: ${error}`);
    }
  }
