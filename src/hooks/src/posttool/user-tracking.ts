 ...
 */
export function userTracking(input: HookInput, ctx?: HookContext): HookResult {
  try {
    ...
    if (toolName === 'Skill') {
      ...
      if (skillName) {
        ...
        (ctx?.log ?? logHook)('user-tracking', `Tracked skill: ${skillName}`, 'debug');
      }
    }
    ...
    if (toolName === 'Task') {
      ...
      if (agentType) {
        ...
        (ctx?.log ?? logHook)('user-tracking', `Tracked agent: ${agentType}`, 'debug');
      }
    }
    ...
    (ctx?.log ?? logHook)('user-tracking', `Error: ${error}`, 'warn');
    return outputSilentSuccess();
  }
}
