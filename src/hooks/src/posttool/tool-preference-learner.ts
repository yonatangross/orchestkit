 ...
 */
export function toolPreferenceLearner(input: HookInput, ctx: HookContext = NOOP_CTX): HookResult {
  ...
  // Reset accumulator
  ctx.log(HOOK_NAME, `Flushed ${pendingUpdates.length} updates (${callsSinceFlush} calls)`);
  pendingUpdates = [];
  callsSinceFlush = 0;
