...
/**
 * Skill Nudge — PostToolUse Bash Hook
 * Issue #705: Contextual nudges for zero-use skills
 *
 * Detects:
 * 1. Successful `git push` → suggests /ork:create-pr
 *
 * Note: /ork:remember nudge is handled by memory-capture.ts (Stop hook) — not duplicated here.
 *
 * Hook: PostToolUse (Bash) — sync, returns additionalContext
 */
...
/**
 * Skill nudge hook — PostToolUse Bash (sync)
 */
export function skillNudge(input: HookInput): HookResult {
  // Nudge: git push success → create-pr
  if (isGitPushSuccess(input)) {
    logHook(HOOK_NAME, 'Nudge: create-pr after git push');
    return outputWithContext('💡 /ork:create-pr — open a PR with validation and auto-filled description.');
  }
