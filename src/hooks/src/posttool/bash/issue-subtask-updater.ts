 ...
 */
export function issueSubtaskUpdater(input: HookInput, ctx?: HookContext): HookResult {
  ...
  (ctx?.log ?? logHook)('issue-subtask-updater', 'Processing git commit for subtask updates...');

  // Check if gh CLI is available
  try {
    execFileSync('which', ['gh'], { stdio: 'ignore', timeout: 2000 });
  } catch {
    (ctx?.log ?? logHook)('issue-subtask-updater', 'gh CLI not available, skipping subtask updates');
    return outputSilentSuccess();
  }
  ...
  try {
    ...
    if (!remote.includes('github')) {
      (ctx?.log ?? logHook)('issue-subtask-updater', 'Not a GitHub repository, skipping');
      return outputSilentSuccess();
    }
  } catch {
    return outputSilentSuccess();
  ...
  if (!issueNum) {
    (ctx?.log ?? logHook)('issue-subtask-updater', 'No issue number found');
    return outputSilentSuccess();
  }

  (ctx?.log ?? logHook)('issue-subtask-updater', `Found issue #${issueNum}, checking for matching subtasks...`);

  // Extract task from commit message
  const commitTask = extractTaskFromCommit(commitMsg);
  if (!commitTask) {
    (ctx?.log ?? logHook)('issue-subtask-updater', 'Could not extract task from commit message');
    return outputSilentSuccess();
  }

  (ctx?.log ?? logHook)('issue-subtask-updater', `Commit task: '${commitTask}'`);

  // Get unchecked tasks from issue
  const uncheckedTasks = getUncheckedTasks(issueNum);
  if (uncheckedTasks.length === 0) {
    (ctx?.log ?? logHook)('issue-subtask-updater', `No unchecked tasks in issue #${issueNum}`);
    return outputSilentSuccess();
  }
  ...
  for (const checkboxText of uncheckedTasks) {
    if (matchesCheckbox(commitTask, checkboxText)) {
      (ctx?.log ?? logHook)('issue-subtask-updater', `Found matching checkbox: '${checkboxText}'`);
  ...
  if (!matched) {
    (ctx?.log ?? logHook)('issue-subtask-updater', `No matching checkboxes found for task: '${commitTask}'`);
    return outputSilentSuccess();
  }
