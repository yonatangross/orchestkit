 ...
 */
export function dependencyVersionCheck(input: HookInput, ctx: HookContext = NOOP_CTX): HookResult {
  // Bypass if slow hooks are disabled
  if (shouldSkipSlowHooks()) {
    ctx.log('dependency-version-check', 'Skipping dependency check (ORCHESTKIT_SKIP_SLOW_HOOKS=1)');
    return outputSilentSuccess();
  }

  ctx.log('dependency-version-check', 'Starting dependency version check');
  ...
  if (!hasPackageJson && !hasRequirementsTxt && !hasPyprojectToml && !hasGoMod) {
    ctx.log('dependency-version-check', 'No package files found, skipping check');
  ...
  if (cached !== null) {
    ctx.log('dependency-version-check', 'Using cached dependency warnings');
    if (cached !== 'none') {
  ...
  if (allWarnings) {
    ...
    ctx.log('dependency-version-check', summary);
    ...
    saveCache(cacheFile, 'none');
    ctx.log('dependency-version-check', 'No known vulnerabilities found');
  }
