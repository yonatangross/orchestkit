 ...
 */
export function prefillGuard(_input: HookInput, ctx?: HookContext): HookResult {
  ...
  try {
    if (isCacheFresh(cacheFile, skillsDir, skillDirs)) {
      ...
      if (cache) {
        (ctx?.log ?? logHook)('prefill-guard', 'Using cached scan results');
  ...
  if (affectedSkills.length === 0) {
    (ctx?.log ?? logHook)('prefill-guard', 'No prefilling patterns detected in skills');
    return outputSilentSuccess();
  }

  (ctx?.log ?? logHook)('prefill-guard', `Found prefilling patterns in ${affectedSkills.length} skills: ${affectedSkills.join(', ')}`, 'warn');
