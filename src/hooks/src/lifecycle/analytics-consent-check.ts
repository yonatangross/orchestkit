 ...
 */
export function analyticsConsentCheck(input: HookInput, ctx?: HookContext): HookResult {
  ...
  if (status.consented) {
    (ctx?.log ?? logHook)('analytics-consent-check', 'User has consented to analytics');
  ...
  if (status.asked) {
    ...
    if (lastEvent && (lastEvent.action === 'declined' || lastEvent.action === 'revoked')) {
      if (shouldShowReminder(lastEvent.timestamp)) {
        ...
        (ctx?.log ?? logHook)('analytics-consent-check', 'Showing 30-day reminder');
        return {
          continue: true,
          systemMessage:
    ...
    (ctx?.log ?? logHook)('analytics-consent-check', 'User recently declined, not prompting');
    return outputSilentSuccess();
  }

  // First time - show a brief notice (not the full prompt, to avoid blocking)
  (ctx?.log ?? logHook)('analytics-consent-check', 'First time user, showing brief notice');
  return {
    continue: true,
    systemMessage:
