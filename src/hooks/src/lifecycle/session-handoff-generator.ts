...
export function sessionHandoffGenerator(input: HookInput, ctx?: HookContext): HookResult {
  try {
    ...
    (ctx?.log ?? logHook)(HOOK_NAME, `Handoff written: ${projectHash}/latest.yaml (${totalTools} tools, ${filesModified} files, status=${status})`);
  } catch (err) {
    (ctx?.log ?? logHook)(HOOK_NAME, `Failed to write handoff: ${err instanceof Error ? err.message : String(err)}`, 'warn');
  }
