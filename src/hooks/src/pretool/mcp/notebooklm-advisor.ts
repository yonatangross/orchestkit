 ...
 */
export function notebooklmAdvisor(input: HookInput, ctx?: HookContext): HookResult {
  ...
  if (WARN_OPS[op]) {
    (ctx?.log ?? logHook)(HOOK_NAME, `WARN: ${op}`);
  ...
  if (op === 'source_add') {
    ...
    if (typeof text === 'string' && text.length > 50000) {
      (ctx?.log ?? logHook)(HOOK_NAME, `WARN: source_add large text (${text.length} chars)`);
  ...
  if (op === 'note' && input.tool_input?.action === 'delete') {
    (ctx?.log ?? logHook)(HOOK_NAME, `WARN: note delete (irreversible)`);
