 ...
 */
export function docstringEnforcer(input: HookInput, ctx?: HookContext): HookResult {
  ...
  if (missingDocstrings.length > 0) {
    ...
    (ctx?.log ?? logHook)('docstring-enforcer', `DOCSTRING_WARN: ${missingDocstrings.length} functions missing docs in ${filePath}`);
  ...
  (ctx?.log ?? logHook)('docstring-enforcer', `DOCSTRING_OK: All public functions documented in ${filePath}`);
  return outputSilentSuccess();
}
