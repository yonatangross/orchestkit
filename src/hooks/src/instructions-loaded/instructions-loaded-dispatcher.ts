...
export function instructionsLoadedDispatcher(input: HookInput, ctx?: HookContext): HookResult {
  const raw = (input as unknown as Record<string, unknown>).files_loaded;
  if (!Array.isArray(raw)) {
    (ctx?.log ?? logHook)(HOOK_NAME, 'No files_loaded in input, skipping');
  ...
  if (filesLoaded.length === 0) {
    (ctx?.log ?? logHook)(HOOK_NAME, 'No valid files in files_loaded, skipping');
    return outputSilentSuccess();
  }

  (ctx?.log ?? logHook)(HOOK_NAME, `Processing ${filesLoaded.length} loaded instruction files`);
  ...
  for (const handler of HANDLERS) {
    try {
      ...
      (ctx?.log ?? logHook)(HOOK_NAME, `${handler.name} failed: ${err}`, 'warn');
    }
  }

  if (contextParts.length === 0) {
    (ctx?.log ?? logHook)(HOOK_NAME, 'All handlers silent');
    return outputSilentSuccess();
  }

  const consolidated = contextParts.join('\n\n');
  (ctx?.log ?? logHook)(HOOK_NAME, `Consolidated ${contextParts.length} handler outputs`);
