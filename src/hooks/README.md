...
> **Matcher-group `if:` keys removed, the documented `if` field is per-hook only (#4060):** hooks.json shipped two `"if"` keys on the **matcher group** (sibling of `matcher`, PreToolUse[7] on `pretool/ask/headless-responder`, PermissionRequest[0] on `permission/unified-dispatcher`). The CC hooks schema has no `if` key at that level in any documented version: the `if` field is defined on the **hook command object** (inside `hooks[]`, next to `type`/`command`; https://code.claude.com/docs/en/whats-new/2026-w13 introduced it there), and even there it "supports only a single permission rule without logical operators" (https://code.claude.com/docs/en/hooks, Common fields). CC 2.1.268 therefore reported both keys as unknown and ignored them on every session start. Nothing in CC 2.1.264 to 2.1.268 changed hook `if` support (doctor matrix #4058: 2.1.264 was never published, and the 2.1.265 to 2.1.268 rows carry no hook-schema item). Both keys are removed. Behaviour is preserved by the mechanisms the schema does support: the `matcher` on each group already scopes its hooks to the tool (`AskUserQuestion`, `Bash`), `headless-responder` self-guards on headless mode and empty question sets, and the unified-dispatcher members self-gate on dangerous input (REJECT_PATTERNS, SECURITY_BLOCKLIST, compound-command rejection). The PermissionRequest condition could never have worked as written even at the documented level: it is an 11-rule `||` chain, and `if` admits one rule; gating the auto-approver to dangerous commands would also have stopped it ever seeing the safe commands it exists to approve, the exact defect #3430/#3438 found on the PreToolUse sibling. This **supersedes the #1941 note below**: that entry said the gates "activated at the 2.1.148 floor"; whatever matched there, a matcher-group `if` is not a schema key, and at the current floor (2.1.251) through 2.1.268 the loader rejects it. A SCHEMA KEYS check in `scripts/validate-registry.mjs` now fails on unknown keys per hook entry against the allowlist (matcher, hooks), with negative controls in `tests/unit/test-hooks-schema-keys.sh`, so the class cannot return silently.
...
| `ORK_SECRET_HOOK` | Mode for `posttool/secret-handler` on Read/Bash/Grep/Glob output. Values: `AUDIT` (default when unset: detect + write `.claude/state/secret-audit.jsonl`, output unchanged), `REDACT` (in-place `[REDACTED:type]` for bounded patterns; gate whole output for critical patterns), `OFF` (no-op). Unknown values resolve to `OFF` so a typo cannot escalate into `REDACT`. | Sidecar log is best-effort and never blocks. Pair with `skill/redact-secrets` (always-on warn) and `security-command-audit` (redacted Bash command log). |
...
(count unchanged at 171, 2026-09-24): `prompt/antipattern-warning`'s `materializeAntipatternRules` no longer writes the project copy of `.claude/rules/antipatterns.md` when the user-global copy (`$CLAUDE_CONFIG_DIR/rules/`, default `~/.claude/rules/`) is byte-identical. Rules files are not free context: CC injects each copy into every prompt, so one copy per project dir multiplied the same text by the number of dirs a session starts in. The skip applies only when the project copy is absent or already identical; a stale project copy is still refreshed (estate-4 review), and hooks never delete rules files. A missing or differing global copy keeps today's write. Header comment corrected: cached is not free. New export `buildAntipatternsContent()` lets tests place an identical global file, and the identical/exists checks live in `lib/log.ts` as `rulesFileMatches`/`rulesFileExists` so the handler keeps no `node:fs` import (#3917 FH-ready policy). 6 cases added (identical global skips the write, identical project copy untouched, stale project copy refreshed, differing global still writes, no global still writes, a stray identical file outside the config dir does not count). No hooks.json registration moved.

(count unchanged at 171, 2026-09-23, #4217 parts 2 and 3): `posttool/secret-handler` defaults to `ORK_SECRET_HOOK=AUDIT` when unset (was OFF for the #1543 rollout); document the three values in the Troubleshooting env table. `agent/security-command-audit` runs Bash `command` through `redactSecretValues()` before append and prefers `CLAUDE_PLUGIN_DATA/logs/security-audit.log` when set (else project `.claude/logs/`). Registry + `SECURITY.md` wording updated so they no longer imply a working secret-audit layer that shipped dead/off/raw. Part 1 (`redact-secrets` object-shaped Bash `tool_response`) already landed in #4330. No hooks.json registration moved.

(count unchanged at 171, 2026-09-22, sc47 F23/F24): `lifecycle/session-cleanup` gains a 14-day age cap on per-session event dirs. `bin/run-hook.mjs` appends one `hook_triggered` row per invocation to `<project>/.claude/memory/sessions/<id>/events.jsonl` (and session-tracker may also write under `CLAUDE_PLUGIN_DATA/sessions`); nothing removed those dirs before, so the tree grew for the life of the checkout. Retention rather than dropping the writer, because `hook-timing.jsonl` is not the same row (project hash `pid`, no session/user id), and `loadSessionEvents()` then `generateSessionSummary()` still consume events.jsonl. Sweep both roots, exact session-id match only, never the ending session, skip dirs with no readable events.jsonl. `bin/stop-uncommitted-check.mjs` (F24) switches from shell `execSync('git ...')` to `execFileSync('git', ['--no-optional-locks','status','--porcelain'], { timeout: 5000 })` and drops the redundant `git rev-parse` probe; stays SYNC in hooks.json because an async Stop `systemMessage` only lands on a next turn that may never exist. Deliberately not `-uno` (untracked is part of the warning contract). No hooks.json registration moved.

(count unchanged at 171, 2026-09-18, #4233 Jev routing seam): `prompt/executor-route-nudge` gains the Jev routing seam behind `ORK_ROUTE_JEV` (off by default; `shadow` records, `steer` names the executor at or above `ORK_ROUTE_JEV_FLOOR`, default 0.5). The seam lives in `lib/route-judgment.ts` (request, redactor ported from the expect lane's `jev_shadow.py` and extended with emails, Israeli phones, nine digit ids, `op://` refs and `clients/` directory names, egress scan in raw and decoded forms, daily token budget plus a 24 h trip on 402 or 429, one 0600 `jev-route.jsonl` record per build-shaped prompt with no prompt text) and `lib/route-classes.ts` (the nineteen classes shared with hq-ext and their executor map; ops classes and the two never-steered classes map to nothing). The handler owns no transport: the seam is injected through a `deps` argument, `shadow` fires the async call and lets the process stay alive for it as the session category lane does, `steer` blocks on a child node process because the dispatcher is synchronous. With the flag unset the handler's behaviour and text are unchanged. No hooks.json registration moved.

(count unchanged at 171, 2026-09-12, plugin eval pilot): `prompt/visual-style-nudge` narrowed and its message rewritten. Measured with `claude plugin eval --ablation with-without` (agent claude-sonnet-5, judge claude-opus-5, 9 cases): two should-not-fire cases scored LOWER with the plugin than without it, -40 on "explain the difference between rebasing and merging" (50 lines of double-line boxes and emoji headers) and -20 on "in one paragraph, explain what a feature flag kill switch is" (three box-framed sections and a bar meter), the second with zero Skill calls, so the hook was the only plugin surface in play. Two causes. `IMPLICIT_SHAPE_RE` fired on `explain`, `summarise` and `walk me through`, which are prose asks; those three are removed and `versus`, `vs`, `difference between` are added, since a comparison is a shape. A new `PROSE_ASK_RE` (`in one sentence|paragraph`, `briefly`, `explain how|what|why`, `define`) silences the implicit path outright; an EXPLICIT ask (`ascii`, `draw`, `vis`) still overrides it. The injected message said "not prose paragraphs", which contradicts rule 2 of `rules/visual-style.md` (prose first, the visual supports the answer); it now restates that rule and the 12-line, 40% budget, and says a conceptual explanation gets no visual. Tests 13 -> 17: the four eval prompts are pinned as silent, a genuine comparison still fires with the new message text asserted, an explicit ask overrides the prose guard, and the existing throttle and delta-skip cases are unchanged. No hooks.json registration moved.

(count unchanged at 171, 2026-09-06, #3951): `posttool/mcp-output-transform` gains `ORK_MCP_REDACT_EXEMPT_TOOLS`, a per-tool exemption from Phase 1 PII redaction. `EMAIL_RE` and `PHONE_RE` are shape-based, and some identifiers are PII-shaped by construction: a WhatsApp JID is `<digits>@c.us` (email-shaped) and a linked-device id is `<digits>@lid` (phone-shaped). Since #2552 the hook has redacted every `mcp__*` result with no exemption of any kind, so a messaging server's own session-status tool had its OWN account id rewritten before the model ever saw it, and a protocol that must target the account's own chat (self-chat, never a hardcoded number) could not resolve its target at all. Measured live on alpha.85, 2026-09-06: `whatsapp_session_status` returned `me.id = "[REDACTED_EMAIL]"`, `lid = "15[REDACTED_PHONE]@lid"`; the new unit suite reproduces exactly that string on the pre-fix source. The env var is a comma-separated list of FULL tool names (`mcp__<server>__<tool>`), read at call time so it can flip within a session, parsed into a `Set` and matched EXACTLY: a tool whose name is a prefix of an exempt entry is not exempt, and neither is one that extends it, so exempting one status tool can never silently exempt a whole server. Scope is deliberately narrow: Phase 2 truncation is untouched (an exempt tool is still truncated, and the reversible-stash path is unchanged), every non-exempt tool on the same server still redacts, and the default (unset) behaviour is byte-identical to before. The trade-off is stated rather than hidden: the exemption is per tool, not per field, so a real email inside an exempt tool's result is passed through too, which is why the operator opts in one tool name at a time. 11 unit cases in `src/__tests__/posttool/mcp-output-transform.test.ts`; 5 fail on main (the exemption assertions) and 6 pass on both (the controls: non-exempt sibling still redacts, prefix and extension not exempt, unset env unchanged, whitespace-only list exempts nothing). The suite's `beforeEach` now also deletes `ORK_MCP_REDACT_EXEMPT_TOOLS`, the #2302 ambient-env isolation applied to the new flag, so an operator who arms it globally does not turn the redaction tests above into passthrough assertions. No hooks.json registration moved.

(count unchanged at 171, 2026-09-06, #3353 register residue): `lifecycle/sweep-stale-worktrees` narrowed. The sibling `<repo>-*` scanning branch (`sweepStaleSiblings`) walked the PARENT directory of the repo on every SessionStart looking for the layout #3319 retired; deleted, together with the `$HOME`/filesystem-root guards that only existed because of it. The hook keeps the `.worktrees/<name>/` shell sweep and the `worktree-pending` marker sweep and no longer reads anything outside the project directory; a test pins that a stale empty sibling survives. `posttool/heartbeat` was NOT removed, against the register row that called `last_heartbeat` dead: the row was corrected instead (the registrar's 24h crash sweep and the socket-less peer fallback both read the column).
...
(count unchanged at 172, 2026-09-01, budget guard goes advisory-only): `pretool/write-edit/context-file-budget-guard` no longer answers `ask` in any permission mode; the #3741 bypass-mode advisory is now the only behaviour. Trigger, measured the same day: both this machine's MEMORY.md indexes sat over the 17,408 B budget (orchestkit 24,202 B, 200 B short of the ceiling where CC stops loading the index at all; hq-ext 23,163 B), both lanes ran in auto mode, and since CC 2.1.211 a hook's `ask` wins over auto mode, so every handoff write in every lane raised a permission dialog. The opinion ("trim the index") is one the model can act on itself, so it is delivered as `additionalContext` through the dispatcher ("not asking, advising: this Write puts MEMORY.md at X KB ... run /ork:dream (index-budget)"); `isBypassMode` and `outputAsk` leave the file. Consumers moved with it: probe `write-memory-budget-ask` is now `write-memory-budget-advisory` (trip -> `context`), `sync-write-edit-dispatcher-verdicts.test.ts` asserts the dispatcher carries the advisory and never a decision (its ask-carry merge keeps working but has no live write-edit producer left), `bypass-mode-ask-gate.test.ts` keeps the guard as a negative pin (does not ask in default mode either). Same day the orchestkit index was compacted with `index-budget.mjs --entry-days 14 --handoff-days 5 --apply`: 24,202 -> 17,346 B, 97 indexed + 210 archived, invariant OK.

(count unchanged at 172, 2026-09-01, legacy-debris sweep): four dead or mislabeled remnants the #3835 purge never covered, found by an outside-in reachability sweep. DELETED: the `@deprecated` no-op `profileInjector()` stub and its `entries/prompt.ts` map slot (no hooks.json entry existed; the real work is `materializeProfileRules()` from `lifecycle/sync-session-dispatcher` at SessionStart, unchanged), the CC < 2.1.88 relative-path compat branch in `permission-denied/project-write-retry.ts` (the floor is 2.1.251; `file_path` is guaranteed absolute), and the legacy `name` worktree-slug field on `HookInput` (WorktreeCreate is not a registered event and no production reader existed). CORRECTED: `entries/prompt.ts` labeled `prompt/thrash-detector` "Legacy... not in hooks.json" while it is registered on UserPromptSubmit, an inverted comment that misled this sweep's own explorer; and this changelog's wave-3 entry said "hence 176 -> 175" in its body against a "175 -> 174" heading (the body figure was wave 2's). NOT deleted, filed as #3867 (EPIC A #3306 child): the deprecated `cli/generate-http-hooks.ts`, which still has entry points (package.json script, doc references). Count unchanged: no hooks.json registration moved. `skill/cross-instance-test-validator` now skips session-edited files outside the project dir before candidate generation (`isUnderProject`). Previously a `/tmp` scratch script recorded in edit-history was validated as repo source: `ancestorsOf` never reaches a projectDir that is not an ancestor, walks to the filesystem root, and the Stop block demanded a doubled-slash `//tests/...` test file, firing exactly when a session does scratch work outside the repo (seen 2026-08-31 during the #3835 rebase surgery). In the same change the "legacy single-file dispatch" branch (reading `tool_input.file_path` + `content`) is DELETED, not hardened: the hook is registered once, on Stop (`hooks.json`), the only entries map carrying it is `entries/skill.ts`, and a Stop payload never carries `tool_input.file_path` (the #3804 rewrite exists because of that). The branch was green-tested dead code, the #959/#3801 shape; its three unit cases go with it, replaced by two #3844 controls (outside-project file yields zero findings; an outside-project file does not mask a real in-project finding). Count unchanged: a branch died, not a registration. `pretool/bash/dangerous-command-blocker` and `pretool/bash/git-validator` deleted. The blocker's DENY patterns are `Bash()` deny rules in the operator payload (every one except the three SQL substrings, inexpressible as rules, loss accepted); its ASK tier (git reset --hard, git clean -fd, git stash drop, force-push to non-protected branches, git checkout ., terraform destroy, sudo) is gone, loss accepted, CC's own permission prompt covers those shapes generically. git-validator's protected-branch deny re-homes to the force-push deny rules plus origin branch protection; its branch-naming and commit-format lints go with it (a JSON rule cannot read the current branch). The count moves 174 -> 172 because two skills declared these hooks in their `hooks:` frontmatter (`commit`, `security-patterns`); both blocks are removed. Runner tests that drove the blocker as a FIXTURE (`run-hook-rewake`, `run-hook-verdict-telemetry`, `run-hook-stdin-timeout`, `input-validation`) now drive `network-egress-guard`'s DENY tier, the surviving Bash deny, with `eval $(curl ...)`; `test-permission-mode-matrix.sh` keeps its load-bearing property ("a deny holds in EVERY permission mode") on the same fixture. Seven security shell files that asserted the blocker's contract are rewritten: four as #3807-shaped tripwires (`test-command-injection`, `test-line-continuation-bypass`, `test-input-validation`, `test-compound-commands`: hook absent, every denied pattern in the payload, canary probe wired into CI; declared STATIC_ONLY in the mutation gate with their reason), three re-pointed to the egress guard (`test-permission-mode-matrix`, `test-additional-security` sections 3/4/6/7/8, `test-unicode-attacks` sections 3/4). Probes: `bash-catastrophic-rm-deny` became `bash-remote-exec-deny`; `bash-force-push-ask` and `bash-pipe-to-shell-deny` retired. `sync-bash-dispatcher`'s roster and ordering comments rewritten to the surviving members. With this wave the purge (#3835) is complete: 29 live blocking hooks audited, 16 deleted across four waves, the KEEP set documented in `shared/rules/cc-native-first.md`.

(count 175 -> 174, 2026-08-31, #3835 purge wave 3): the security baseline changes, then two guards go. First, in its own reviewed commit, `dangerous-command-blocker`, `file-guard` and `git-validator` leave `src/hooks/src/lib/security-hooks-registry.ts` (6 -> 3), the P0 priority table and the runner's un-disableable set: their opinions are `permissions.deny` rules in the OPERATOR's settings scope (setup phase 3.6 writes them with consent, doctor audits them, `tests/ci/restricted-smoke/probe-permission-deny.sh` gates them in CI), which CC enforces even under `--dangerously-skip-permissions`; a plugin settings.json cannot carry them, so "un-disableable hook" was never the strongest guarantee available. Then `pretool/write-edit/file-guard` (its PROTECTED_PATTERNS ported to payload v2 as `Edit()`/`Write()` deny rules for credentials.json, secrets.json, *.pem, private.key, id_rsa, id_ed25519, .husky/, before deletion) and `pretool/read/credential-read-guard` (the one hooks.json entry in this wave, hence 175 -> 174, matching this entry's heading; an earlier revision of this sentence said 176 -> 175, which was wave 2's move, fixed 2026-09-01; it existed precisely because plugin-shipped `Read()` rules were inert, and it goes now that the operator-scope delivery exists and is probed) are deleted. The four security shell files that asserted file-guard's contract (`test-path-traversal.sh`, `test-symlink-attacks.sh`, `test-unicode-attacks.sh` section 2, `test-additional-security.sh` probes) are rewritten in the #3807 tripwire shape: the hook must stay gone AND every pattern it denied must remain in the payload, with the CI canary probe named as the native layer's watchdog; the full old contracts are in git history at the commit before this one. Entries-map 197 -> 195 on this branch; probes 18 -> 16 (`write-file-guard-oversize-ask`, `read-credential-file-deny` retired).
...
(count unchanged at 176, 2026-08-31, #3741): `pretool/write-edit/context-file-budget-guard` no longer goes silent in bypass mode. Its early return on `isBypassMode()` was reasoned as "an ASK with nobody to ask is noise", but the sessions that write MEMORY.md most are the long autonomous bypass-mode ones, and measured on this machine six projects sat at or over the 17,408 B budget with no signal at all (this repo's index: 22,234 B, 127.7%). In bypass mode the guard now never asks and instead returns advisory context through `outputPreToolAdvisory` ("bypass mode, not asking: this Write puts MEMORY.md at X KB, Y KB over the Z KB budget. Trim, or run /ork:dream (index-budget) before writing more"); context is the only channel that reaches the model there, since `sync-write-edit-dispatcher` merges `additionalContext` and drops `systemMessage` (the #3801 class). The header's "~24 KB hard read limit" is now stated as unmeasured (two probes read past it). Paired skill-side change in the same PR: dream's rebuild invariant is now two-file (`indexed(MEMORY.md) + indexed(MEMORY-ARCHIVE.md) == files_on_disk`, disjoint, exact-case links, one trailer), a new STEP 5.5 runs `skills/dream/scripts/index-budget.mjs` (report by section, entries over 400 chars, rule-based demotion proposals: Handoffs older than 7 days, Feedback/Project older than 30 days, never hub_/Policy/user_/modified within 7 days; `--apply` behind dream's ask gate with `.MEMORY.md.prev` rotation and a re-verified invariant), and the stale "200 lines" rule is gone. Verdict probe `context-file-budget-guard-bypass` (trip: bypass + 18 KB Write -> `context`; control: small Write -> `silent`); unit cases pin the advisory text and the never-ask rule; `tests/skills/test-dream-index-budget.sh` pins the script on synthetic fixtures (21 cases) and fails on main.
