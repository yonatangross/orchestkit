...
|----------|------|--------|-------------|
| Audit Checks | `${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/rules/audit-checks.md` | HIGH | What to validate per skill |
| Status Rules | `${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/rules/audit-status.md` | MEDIUM | PASS/WARN/FAIL classification |
| Output Format | `${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/references/output-format.md` | MEDIUM | Table layout and column definitions |
| Edge Cases | `${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/references/edge-cases.md` | LOW | Manifest "all", orchestration skills |
...
2. **Check each skill** — Run all checks from `Read("${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/rules/audit-checks.md")` in parallel
3. **Classify** — Apply status rules from `Read("${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/rules/audit-status.md")`
4. **Render** — Output table using format from `Read("${CLAUDE_PLUGIN_ROOT}/skills/audit-skills/references/output-format.md")`
