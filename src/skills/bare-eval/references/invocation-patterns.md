...
>
> **CC 2.1.205**: An invalid `--json-schema` now **hard-errors** instead of silently
> producing unstructured output — a malformed grading/trigger/quality schema fails
> loudly at invocation rather than yielding un-parseable results downstream. Validate
> schemas before batch runs. The `format` keyword is now accepted; ork's grading
> schemas use `format` only as a property name (not the JSON-Schema keyword), so this
> is a no-op for existing schemas but safe to adopt.
