...
```python
async def self_consistent_answer(question: str, n_paths: int = 5, model_thinks: bool = False) -> str:
    """Sample several reasoning paths and vote on the answer.

    model_thinks=True for models that reason by default (Claude Opus 5.5 always
    thinks and rejects sampling params): send the bare question, no CoT suffix,
    no temperature. Non-thinking models get the CoT suffix and a higher
    temperature for path diversity.
    """
    content = question if model_thinks else f"{question}\n\nThink step by step."
    sampling = {} if model_thinks else {"temperature": 0.7}
    answers = []
