...
```bash
# At our floor (CC ≥ 2.1.168): plan mode survives resume as declared
...
# At our floor (CC ≥ 2.1.168) — works as documented
claude --add-dir Z:\share\project
...
No setting change required — the JSON field and env var are always present at our floor (CC ≥ 2.1.168). Hook authors should treat absence as `medium` for forward compat.
...
```typescript
// Multi-select question that now works at our floor (CC ≥ 2.1.168)
AskUserQuestion({
  questions: [{
