...
```typescript
// Package is `emulate`, and the entry point is createEmulator.
...
describe('GitHub PR workflow', () => {
  ...
  beforeAll(async () => {
    ...
    // Point tests at emulate. The instance exposes `url`, not a bare port.
    process.env.GITHUB_API_BASE = emulator.url;
  });
