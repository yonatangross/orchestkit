...
| Category | What It Checks | Reference |
|----------|---------------|-----------|
| **4. Memory** | .claude/memory/ graph integrity + queue depth; auto-memory MEMORY.md index budget; per-agent `.claude/agent-memory/<agent>/MEMORY.md` (orphans, staleness, 150-line warn, secret/PII) | load `references/memory-health.md` and `references/agent-memory-dir.md` |
