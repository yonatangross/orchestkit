...
✗ Hooks: dist/pretool.mjs missing - run: cd src/hooks && npm run build
...
Per-agent `memory:` dirs are a separate check. Load `references/agent-memory-dir.md` and run:

```bash
bash src/skills/doctor/scripts/check-agent-memory.sh \
  --memory-root .claude/agent-memory \
  --agents-dir src/agents
```

Pass `--activity-file` (name, tab, unix epoch) only when you know the agent ran. Without it, staleness is skipped so an idle agent is not reported stale.
...
# Peer skills from other marketplaces (#4233): the TypeSafe agent skill that the
# Jev routing seam (ORK_ROUTE_JEV) and the session category provider build on.
# Local files only (known_marketplaces.json, installed_plugins.json, the
# marketplace clone's manifest); no network. Five named verdicts, exit 1 when
# a fix applies, and the fix text is the vendor's own install or update command:
...
python3 "${CLAUDE_PLUGIN_ROOT}/skills/doctor/scripts/check-peer-skill.py" --plugin typesafe@typesafe-ai || true
