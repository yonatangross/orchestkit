...
| cross-session `SendMessage` + `ListAgents` | 2.1.224 | Sessions can message each other across your machines; `ListAgents` discovers them. **Counts corrected 2026-08-13**: the "34 of 36 grant `SendMessage`, 0 grant `ListAgents`" figure recorded here on 2026-08-08 is stale. Measured at HEAD: **23 of 37 agents grant `SendMessage`, 19 grant `ListAgents`**, and **4 grant `SendMessage` without `ListAgents`** (`code-quality-reviewer`, `component-curator`, `eval-runner`, `security-auditor`). Those four can send but cannot enumerate a legitimate target (#3316). Delivery is per-session-socket on one machine, via Anthropic servers across machines | Agents send to guessed names with no roster to scope against |
...
| `ListAgents` marks `offline` / `cloud` | 2.1.229 | Disconnected Remote Control sessions are marked `offline` and your cloud sessions labeled `cloud`. **This is the liveness primitive #3316 was missing**: a sender can now scope to live peers instead of messaging a dead name. Sharpens the 4-agent gap in the 2.1.224 row above: `code-quality-reviewer`, `component-curator`, `eval-runner`, and `security-auditor` grant `SendMessage` without `ListAgents`, so they cannot make this check | A roster entry gave no signal whether the peer was alive |
