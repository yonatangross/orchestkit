...
| hook `continue:false` halt no longer dropped | 2.1.212 | A blocking hook's halt is honored even when the tool call fails or completes mid-stream, and hook-infra errors are no longer misreported as user rejections. ork's blocking hooks (`skill/merge-readiness-checker.ts`, SessionEnd/SubagentStop dispatchers) now block reliably; no ork change | `continue:false` could be silently dropped mid-stream |
...
| gateway keep-alive vs stream idle timeout | 2.1.222 | The stream idle timeout no longer fires on custom `ANTHROPIC_BASE_URL` gateways while server keep-alive pings are arriving on the wire. ork sets `CLAUDE_STREAM_IDLE_TIMEOUT_MS: 180000` in `src/settings/ork.settings.json`; gateway users no longer need to raise it further to work around spurious idles | Long agent runs killed on gateways despite live keep-alives |
...
| workflow `import()` sandbox escape closed | 2.1.223 | Workflow scripts can no longer use dynamic `import()` to run code outside the workflow sandbox. ork ships 3 workflow scripts (`audit-full-mapreduce.mjs`, `skill-fitness.mjs`, `heal-loop.mjs`). **Verified: zero `import(` across all three**, so nothing regresses. Treat dynamic `import()` as unavailable when authoring new workflow scripts | A workflow script could execute outside its sandbox |
...
| cross-session `SendMessage` + `ListAgents` | 2.1.224 | Sessions can message each other across your machines; `ListAgents` discovers them. **34 of 36 ork agents grant `SendMessage`, 0 grant `ListAgents`**, so every agent can send, none can enumerate a legitimate target (#3316). Delivery is per-session-socket on one machine, via Anthropic servers across machines | Agents send to guessed names with no roster to scope against |
...
| `denyRead` trailing-slash bypass closed | 2.1.224 | Sandbox filesystem deny entries written with a trailing slash (`denyRead: "~/.aws/"`) were silently bypassable on Linux and macOS. **Verified no-op for ork's shipped values**: `src/settings/ork.settings.json` uses `["~/.aws/credentials", "~/.ssh/*", "~/.gnupg/*"]` and `doctor/references/sandbox-posture.md` recommends `["~/.ssh", "~/.aws", "~/.config/gh"]`, no trailing slash anywhere. Keep it that way when #3311 adds the credential denies | A trailing slash turned a deny rule into a no-op |
