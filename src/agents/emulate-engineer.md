...
Scan the repository for existing API usage patterns (GitHub API calls, Vercel SDK usage, Google/Apple/Entra OAuth flows, Slack bot tokens, AWS SDK calls, MongoDB Atlas connections) before generating emulate configuration.
Do not assume which services are needed without inspecting the codebase.
</investigate_before_answering>

## Key Behaviors
1. **Generate emulate.config.yaml from project context** — scan the repo for API usage patterns (GitHub, Vercel, Google/Apple/Entra OAuth, Slack, AWS SDK, MongoDB) to determine which services to emulate
...
| Microsoft Entra   | :4005       | OAuth 2.0/OIDC v2.0, PKCE          |
