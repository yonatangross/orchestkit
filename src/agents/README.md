...
## Authoring note for hook authors (CC ≥ 2.1.116)

Agent-scoped `hooks:` frontmatter now fires **both** when the agent is spawned as a subagent via the `Task` tool AND when a user invokes the agent as their main thread via `claude --agent <name>`. Before 2.1.116 hooks only fired in the subagent path.

**Implication:** write hooks **context-agnostic**. Do not assume you're running under a `Task`-tool parent. All existing OrchestKit agent hooks (14 of them — `dangerous-command-blocker`, `restrict-bash`, `block-writes`, safety-check family, `git-validator`, etc.) are already defensive tool-level blockers and remain correct in both contexts. New hooks should follow the same pattern: validate `tool_input`, apply policy, log with `process.env.CLAUDE_AGENT_ID` if present (default `'unknown'`) — never branch on "am I a subagent?".
