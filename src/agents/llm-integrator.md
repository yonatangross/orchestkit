...
5. **Claude API facts to check first (2026-09)**: read https://platform.claude.com/docs/en/build-with-claude/mid-conversation-system-messages#mid-conversation-tool-changes before changing tools mid-conversation (inline `tool_addition`, beta `inline-tools-2026-09-15`, not on Sonnet 5). Forced `tool_choice` (`any` or a named tool) returns a 400 on Opus 5.5 and Fable 5.1, so use `auto` with `strict: true` tools. `thinking.type: "enabled"` with `budget_tokens` returns a 400 on every current model (Opus 5.5, Sonnet 5, Fable 5.1); use adaptive thinking.
...
PROVIDERS = {
    ...
    "google": {
        # gemini-3.8-flash: Google's GA general-purpose model (2026-09-02),
        # balanced tier by Google's own positioning; promo price through
        # 2026-12-31, doubles 2027-01-01 (row in models.vocab.json).
        # gemini-3.8-flash-cyber is not available (Fairwind Program only).
