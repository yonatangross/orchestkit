...
| `/clear` | Clear conversation and reset cached skills (preserves background agents since 2.1.72) | CC 2.1.63 |
| `/fast` | Toggle fast mode (same model, faster output) | CC 2.1.59+ |
| `/loop` | Run prompt/command on recurring interval (e.g. `/loop 5m /foo`) | CC 2.1.71 |
| `/effort` | Set reasoning effort: low/medium/high, or `auto` to reset | CC 2.1.72 |
| `/plan` | Enter plan mode, optionally with description (e.g. `/plan fix auth bug`) | CC 2.1.72 |
