...
>
> **Tip (CC 2.1.116+):** `/reload-plugins` and background plugin auto-update now **auto-install missing plugin dependencies** from marketplaces you've already added. If a plugin fails to load because of a missing dep, `/reload-plugins` resolves it in place — no manual `plugin install` step needed.
