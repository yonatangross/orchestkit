...
**Result:** Exactly 1 hook injects volatile per-turn context: `pipeline-detector`, which is genuinely volatile (depends on current turn state). The audit originally counted 2, but `context-exhaustion-warner` was never volatile in practice — its StatusLine bridge file lost its writer in `2de086fa9` (#1194) and it silently no-op'd on every turn from 2026-03-29 until #3427 deleted it (#3321 claim 4). No further migration candidates identified.
