...
(count unchanged at 171, 2026-09-24): `prompt/antipattern-warning`'s `materializeAntipatternRules` no longer writes the project copy of `.claude/rules/antipatterns.md` when the user-global copy (`$CLAUDE_CONFIG_DIR/rules/`, default `~/.claude/rules/`) is byte-identical. Rules files are not free context: CC injects each copy into every prompt, so one copy per project dir multiplied the same text by the number of dirs a session starts in. An existing identical project file is left in place, never deleted from a hook; a missing or differing global copy keeps today's write. Header comment corrected: cached is not free. New export `buildAntipatternsContent()` lets tests place an identical global file, and the identical check lives in `lib/log.ts` as `rulesFileMatches` so the handler keeps no `node:fs` import (#3917 FH-ready policy). 5 cases added (identical global skips the write, identical project copy untouched, differing global still writes, no global still writes, a stray identical file outside the config dir does not count). No hooks.json registration moved.
