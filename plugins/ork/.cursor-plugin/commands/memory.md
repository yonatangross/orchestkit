...
> **Cross-session read strategy (Opus 5.5 / CC 2.1.111+):** Opus 5.5 reads filesystem memory reliably across sessions. When loading context at session start, prefer the layered read order:
...
Load details: `Read("skills/memory/references/memory-commands.md")` for full usage, flags, output formats, and context-aware result limits for each subcommand.
...
| `viz` | Render knowledge graph as Mermaid diagram. See also `Read("skills/memory/references/mermaid-patterns.md")` |
...
Cron job in `.github/workflows/memory-staleness.yml` runs nightly at 04:00 UTC. Calls `skills/memory/scripts/staleness_cron.py` to scan ALL memory MCP entities, flag those whose latest observation timestamp is older than 30 days, and write a Markdown report to `docs/reports/memory-staleness-YYYY-MM-DD.md`.

```bash
python3 skills/memory/scripts/staleness_cron.py docs/reports/ \
...
|------|--------|----------------|
| entity-extraction-patterns (load `skills/memory/rules/entity-extraction-patterns.md`) | HIGH | Entity types, relation types, graph query semantics |
| deduplication-strategy (load `skills/memory/rules/deduplication-strategy.md`) | HIGH | Edit-over-Write pattern, anchor-based insertion, verification |


## Session Resume

Load details: `Read("skills/memory/references/session-resume-patterns.md")` for CC 2.1.31 resume hints, context capture before ending, and resume workflows for PRs, issues, and implementations.
