...
Load details: `Read("skills/setup/references/scan-phase.md")` for full scan commands (20 parallel Glob probes + dependency file reads + pattern detection counts).
...
Load details: `Read("skills/setup/references/safety-check.md")` for the full AskUserQuestion prompt and conflict detection logic.
...
Load details: `Read("skills/setup/references/configure-wizard.md")` for the full 6-step interactive configuration flow (branch strategy, commit scope, localhost browser, perf telemetry, log verbosity, webhook telemetry) and env var reference.
...
Present skill categories using `AskUserQuestion` with 4 focus options (Full-stack, Backend, Frontend, DevOps) with `multiSelect: true`. Load `Read("skills/setup/references/stack-skill-mapping.md")` for mapping tables and custom skill suggestions.

## Phase 5: MCP Recommendations

Check installed vs recommended MCPs by reading `.mcp.json` and `~/.claude/settings.json`. Load `Read("skills/setup/references/stack-skill-mapping.md")` for the MCP recommendation matrix. Present as toggles with install commands.
...
Compute a composite score (0-10) from 6 dimensions. Load `Read("skills/setup/references/readiness-scoring.md")` for dimension weights, score presentation template, memory integration, and improvement plan template.
...
Load details: `Read("skills/setup/references/claude-md-health.md")` for analysis steps, thresholds, @import syntax, and `.claude/rules/` path-scoped rules.
...
Load details: `Read("skills/setup/references/telemetry-setup.md")` for the full configuration flow.
...
Load details: `Read("skills/setup/references/integrations.md")` for setup steps.
...
Both are session-scoped trial paths; for permanent install, use the standard `/plugin install` flow against a marketplace. See `skills/configure/references/cc-version-settings.md` § CC 2.1.128/2.1.129 for the full notes.
...
Load preset details: `Read("skills/setup/references/presets.md")`
