...
Load `Read("skills/explore/rules/exploration-agents.md")` for Agent tool mode prompts.

Load `Read("skills/explore/rules/agent-teams-mode.md")` for Agent Teams alternative.
...
Load `Read("skills/explore/rules/code-health-assessment.md")` for agent prompt. Load `Read("skills/explore/references/code-health-rubric.md")` for scoring criteria.

### Phase 6: Dependency Hotspot Map

Load `Read("skills/explore/rules/dependency-hotspot-analysis.md")` for agent prompt. Load `Read("skills/explore/references/dependency-analysis.md")` for metrics.

### Phase 7: Product Perspective

Load `Read("skills/explore/rules/product-perspective.md")` for agent prompt. Load `Read("skills/explore/references/findability-patterns.md")` for best practices.

### Phase 8: Generate Report

Load `Read("skills/explore/references/exploration-report-template.md")`.
...
1. Load the format and catalog: `Read("skills/explore/references/dashboard-spec.md")`. Reference example: `skills/explore/references/dashboard-example.json`.
...
```bash
node "skills/explore/scripts/render-spec.mjs" .claude/chain/explore-dashboard.json --check
```
...
```bash
node "skills/explore/scripts/render-spec.mjs" .claude/chain/explore-dashboard.json
```
...
After the session synthesis lands, optionally invoke `skills/explore/scripts/post_explore_summary.py <session-dir>` to auto-emit a notebook-backed summary of the exploration. Self-skips on every non-happy-path so it never breaks the run:

```bash
python3 skills/explore/scripts/post_explore_summary.py "$CLAUDE_JOB_DIR"
```
