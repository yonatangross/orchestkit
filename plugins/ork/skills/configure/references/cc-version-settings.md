...
//   --worktree
//   EnterWorktree tool
//   agent-isolation worktrees (Agent tool, isolation: "worktree")
...
```json
{ "availableModels": ["claude-opus-5-5", "claude-sonnet-5", "claude-haiku-4-5"] }
```
...
CC 2.1.243 added two settings so API-key and cloud-provider users can keep a 1-hour prompt cache on the main conversation while subagents stay at 5 minutes. On a subscription there is nothing to set, and the subagent half was settled by measurement rather than policy (#3876, 2026-09-02, 14 days of one workstation's transcripts, 170K assistant turns):

| kind | writes already on 1h TTL | turns after a 5 to 60 min gap | share of write tokens |
|---|---|---|---|
| main sessions | 94% (setting absent) | 2.0% | 7.6% |
| subagents | 0.01% | 0.2% | 2.8% |

A 1-hour TTL pays only when re-writes after 5 to 60 minute gaps exceed about 38% of all write tokens (1h write 2x input, 5m write 1.25x, read 0.025x; the same 38 to 40% for Opus tiers). The eviction is real (in the 5 to 15 minute bucket subagent writes outrun reads five to one) and still too rare to pay for. Re-measure with `scripts/measure-cache-gaps.py --days 14` if subagents start idling (long Monitor waits inside a fan-out); API-key users who see 5m writes on the main conversation are the one audience `promptCacheTtl: "1h"` is for.
...
CC 2.1.269 adds a diff of files changed by a Bash command to the Bash tool result when Bash handles file edits. Enable `bashEditDiffEnabled` when the changed file diff should accompany that result.

**Action for OrchestKit**: leave it as an operator choice. ork does not set user configuration.

### `OTEL_METRICS_INCLUDE_REPOSITORY`, repository telemetry attributes

CC 2.1.269 adds `OTEL_METRICS_INCLUDE_REPOSITORY` to tag OpenTelemetry metrics and events with `vcs.*` repository attributes. Commit events also receive `vcs.ref.head.*` when `OTEL_LOG_TOOL_DETAILS` is set.

**Action for OrchestKit**: set this only in the environment that owns the telemetry policy. ork ships no OTEL environment configuration.

### `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`, Workflow fan out

CC 2.1.269 adds `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` to set a Workflow run's concurrent agent limit within 1 to 256 for inference bound fan outs.

**Action for OrchestKit**: keep the default unless the workflow and provider capacity have been measured for a higher fan out.

### `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS`, gateway discovery timeout

CC 2.1.269 adds `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` to extend the LLM gateway `/v1/models` discovery timeout from its 3 second default.

**Action for OrchestKit**: use this only for a gateway whose model discovery needs more than 3 seconds. It does not enable model discovery by itself.

### `CLAUDE_CODE_BG_TASKS_REPORT_RUNNING`, background task reporting

CC 2.1.269 fixes remote and headless sessions reporting that they are waiting for input while background agents are still running. Set `CLAUDE_CODE_BG_TASKS_REPORT_RUNNING=0` to restore the earlier behavior.

**Action for OrchestKit**: leave it unset unless a caller depends on the earlier reporting behavior.

### Per-command `allowed_domains` in sandboxed auto mode
