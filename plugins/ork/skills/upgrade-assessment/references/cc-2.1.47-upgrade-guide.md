...
### Runtime Utilities
- `cc-version-matrix.ts`: CC version constants (`MIN_CC_VERSION`, `LATEST_KNOWN_CC`) + `compareCCVersions()`. (The per-feature catalogue + `hasFeature`/`getAvailableFeatures` helpers were removed in #2229 — they had zero production callers; ork adopts features by use, not runtime gating.)
