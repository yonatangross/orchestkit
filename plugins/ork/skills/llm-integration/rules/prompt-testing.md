...
async def self_consistent_answer(
    question: str,
    n_paths: int = 5,
    model_thinks: bool = True,
    accepts_sampling: bool = False,
) -> str:
    """Sample several reasoning paths and vote on the answer.

    model_thinks controls only the CoT suffix: models that reason by default
    (Claude Opus 5.5 always thinks) get the bare question.
    accepts_sampling controls only temperature. Current Claude models (Opus 5.5,
    Opus 5, Opus 4.8/4.7, Sonnet 5, Fable 5/5.1) return 400 on temperature,
    top_p and top_k even with thinking off, so it defaults to False; set it True
    only for a model older than the Opus 4.7 generation (the claude-api skill's
    Thinking and Effort table lists them) or a non-Claude model.
    On current Claude models, path diversity comes from independent calls and
    the effort setting, not temperature.
    """
    content = question if model_thinks else f"{question}\n\nThink step by step."
    sampling = {"temperature": 0.7} if accepts_sampling else {}
    answers = []
    for _ in range(n_paths):
        response = await llm.chat([{"role": "user", "content": content}], **sampling)
