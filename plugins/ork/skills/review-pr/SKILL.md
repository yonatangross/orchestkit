...
CC 2.1.202 reverted `/review` to a **fast single-pass** review — a quick "are there bugs in this diff?" gate. The built-in **multi-agent** review now lives at `/code-review <level> <pr#>`. Use `/review` for a fast single-pass review, or `/code-review <level> <pr#>` when you want CC's own multi-agent sweep at a chosen effort level. Since CC 2.1.218, `/code-review` runs as a **background subagent**: the review no longer fills your conversation, results arrive when it completes, and slash commands stacked after it keep it as their review target — so don't wait inline for its output the way pre-2.1.218 docs assumed (#3092).
