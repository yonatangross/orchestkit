...
0. `/ork:verify` Phase 2 --> **Workflow** by default (`workflows/verify-dispatch.js`); the rules below apply when it falls back to Agent Teams or the Agent tool. `/ork:assess` keeps these rules until its own workflow ships.
