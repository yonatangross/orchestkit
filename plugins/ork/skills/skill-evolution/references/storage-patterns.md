...
The `lifecycle/rules-size-check` hook (#1815) warns at 35k chars (WARN) and 38k chars (CRITICAL) for any **single** auto-loaded file at the top level of `.claude/rules/`. Operator-facing stderr signal on every SessionStart.

**Caveat — the hook does NOT catch the #2589 failure mode.** As of this writing it scans only the top level of `.claude/rules/` (no recursion) and checks each file's size *individually*, not the aggregate. So 95 per-entry files of ~7k each under `.claude/rules/decisions/` — summing to ~200k loaded chars — slip past it entirely: each file is under 35k, and the subdirectory is never scanned. CC, however, loads them all. That asymmetry is a second, independent reason to keep per-entry files in `docs/decisions/`: there is no detection safety net for bloat hidden in `.claude/rules/` subdirectories. (Hardening the hook to recurse and sum the aggregate against a `.claude/rules/**` budget — as `yonatan-hq/platform#5468` did with `check-rules-autoload-budget.py` — is the durable fix; tracked separately.)
