...
field whose name depends on the line format. Read them first. Run each group
only for its configured endpoint. For Loki, ask the user for a verified stream
selector from their deployment and set `ORK_OTEL_LOKI_SELECTOR` to it. Do not
assume a label called `source` or a value called `cc-otel`. If no selector is
available, keep Loki panels on local JSONL or mark them unknown.

```bash
if [ -n "${ORK_OTEL_PROM_URL:-}" ]; then
...
if [ -n "${ORK_OTEL_LOKI_URL:-}" ] && [ -n "${ORK_OTEL_LOKI_SELECTOR:-}" ]; then
  query_option=-data-urlencode
  curl -sG "$ORK_OTEL_LOKI_URL/loki/api/v1/query_range" "-$query_option" "query=$ORK_OTEL_LOKI_SELECTOR" \
    "-$query_option" 'limit=1' | jq -r '.data.result[0].values[0][1]' | jq 'keys, (.attributes // {} | keys)'
fi
```

Substitute what Step 1 printed into the queries below. The names below are the
common spelling; they are templates, not facts. Replace `<verified-stream-selector>`
with the exact verified selector supplied in Step 1 before using a LogQL query.
