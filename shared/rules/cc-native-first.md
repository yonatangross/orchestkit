...
|---------------|----------------------|--------------------|
| `lifecycle/session-registrar.ts` + `session-finalizer.ts` + `posttool/heartbeat.ts` (sessions.db) | `claude agents --json` liveness (2.1.161/162) | 🟢 **KEEP**, re-verified 2026-09-06 (#3353, PR #3943). `sessions` is the **enforced-FK parent** (`PRAGMA foreign_keys=ON`) of `skill_invocation`; `locks`/`settings_overrides`/`worktree_links` were dropped in migration 005 (0 rows ever, no writer), and `enter-registrar.ts` had already gone with #3315, so the two-phase-insert reason no longer exists. The earlier claim here that `last_heartbeat` is a dead column was wrong at HEAD: `session-registrar.ts` `sweep()` marks `running` rows `crashed` when `last_heartbeat < now - 24h`, and `isPeerLive()` falls back to it for rows with no messaging socket. `posttool/heartbeat.ts` is the only writer that moves the column after insert, so deleting it would reap live sessions older than a day (1 of 25 running rows on 2026-09-06). Both stay. |
...
| `network-egress-guard` DENY tier | none | 🟢 **KEEP, measured 2026-09-06 (#3877 step 3, lane 5)**: judges executing fetched bytes (`bash <(curl …)`, `eval $(curl …)`, `nc -e`; the plain `curl \| sh` shape is an operator-scope `Bash()` deny rule since the #3835 purge). The containment probe on CC 2.1.263, ork off, auto mode, `example.com` in `sandbox.network.allowedDomains`: `curl -sS https://example.com/nonexistent.sh \| sh` EXECUTED, `permission_denials: []`, no `<sandbox_violations>`, sh failed only on the HTML body. CC's sandbox judges the host, never what the bytes do once fetched, so this tier has no native equivalent. Transcript: yonatangross/orchestkit#3877 (issuecomment-5560987568). Headless `claude -p` arm; the interactive arm was not run. |
...
| `read/credential-read-guard` | same `Read()` deny rules | ✅ **DELETED 2026-08-31** (#3835 wave 3, hooks 175 -> 174; row stamped 2026-09-06 in lane 5 of #3877). The "after probe" condition was met by the CI canary `tests/ci/restricted-smoke/probe-permission-deny.sh` and the #3836 doctor audit before the deletion landed; this row had stayed in the future tense for six days after the file was gone. |
