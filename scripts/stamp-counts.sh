...
sync_versions() {
  ...
  # Harness manifests (#2528 close-out, 2026-09-06). Every plugin manifest a
  # host reads, source or built, is stamped from package.json HERE, in the
  # build, never by hand. Before this loop only manifests/ork.json fed the
  # built ork manifests, and the two Codex SOURCE files below had no writer at
  # all: measured on origin/main 2026-09-06, manifests/codex/ork-codex.json and
  # src/codex/ork-codex/.codex-plugin/plugin.json still said 9.5.4 while
  # package.json said 10.0.0-alpha.8x (docs/audits/codex-alignment-readonly-
  # 2026-09-06.md). The built files are restamped too so that ONE build pass
  # converges even when a source manifest was stale when build-plugins.sh
  # generated them earlier in the same run. Absent built files are skipped
  # (stamp-counts also runs from tests/unit/test-sync-versions.sh's sandbox
  # before any build); tests/unit/test-sync-versions.sh asserts every present
  # one equals package.json and tests/ci/fault-arms/sync-versions.sh proves a
  # hand-edited 9.5.4 fails that test.
  local harness_manifest
  for harness_manifest in \
      ...
      plugin.json; do
    local f="$PROJECT_ROOT/$harness_manifest"
    if [[ ! -f "$f" ]]; then continue; fi
    jq --arg v "$version" '.version = $v' "$f" > "${f}.tmp" && mv "${f}.tmp" "$f"
    synced=$((synced + 1))
  done

  echo "Version: ${version} synced across ${synced} sibling files"
}
