...
echo ""
echo "To create the Release KB, ask Claude to run the release-notebook skill:"
echo ""
echo '  "Create a release notebook for this version"'
echo ""
echo "Or run these MCP calls manually inside a Claude Code session:"
echo ""
echo "  # 1. Create notebook"
echo "  mcp__notebooklm-mcp__notebook_create(title=\"${NOTEBOOK_TITLE}\")"
echo ""
echo "  # 2. Add sources (replace NOTEBOOK_ID with the returned ID)"
echo "  mcp__notebooklm-mcp__source_add(notebook_id=NOTEBOOK_ID, type=\"text\","
...
echo "  mcp__notebooklm-mcp__studio_create(notebook_id=NOTEBOOK_ID, type=\"audio_overview\")"
echo ""
echo "  # 4. Share with project owner"
echo "  mcp__notebooklm-mcp__notebook_share_invite(notebook_id=NOTEBOOK_ID,"
