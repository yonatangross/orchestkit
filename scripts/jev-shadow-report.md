...
For promotion, sample every confidence band and oversample high-confidence disagreements. Label failures using downstream results. For each decision type, sweep floors on a held-out calibration set and choose the lowest floor meeting the stated precision bound. Freeze that floor for a rollout window with a recorded expiry, report coverage on independent held-out tasks, and retune only between windows using a fresh held-out set. Keep browser completion correctness, extra actions, fallback quality, latency and cost as separate measures. Record stale snapshot and invalid-action handoffs as regression cases. Agreement alone cannot justify shadow to steer.
...
This fingerprint enforcement applies to weak sidecars. The existing manual
adjudication format still joins by source path and line; use frozen files for
those labels and keep them separate from weak sidecars.
