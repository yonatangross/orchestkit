...
1. Reads the literal `KNOWN_ENV_NAMES` list in `src/env-names.ts` (generated from the platform `.env.tpl` op:// lines, names only). The list is embedded as string literals because the plugin validate contract requires every `$.env.get` call site to carry a literal name; a name read from JSON at runtime fails the contract and masks nothing when the file is missing. Registration fails loud if the list is empty.
2. For each name, calls `$.env.get("NAME")` with that literal - non-empty values >= 8 chars enter the mask table
...
- `src/env-names.ts` - literal env var names (generated by `scripts/gen-env-names.sh` from the platform `.env.tpl`; set `ENV_TPL` and run it to regenerate)
- `scripts/gen-env-names.sh` - generator script; fails loud when the template is missing or has no op:// lines
