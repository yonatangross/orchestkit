...
- **Model Path**: !`grep -r "MODEL_PATH\|model_name" .env* 2>/dev/null | head -1 | cut -d'=' -f2 || echo "unsloth/Meta-Llama-3.1-8B"`
...
model:
  name: !`grep -r "MODEL_PATH\|model_name" .env* 2>/dev/null | head -1 | cut -d'=' -f2 || echo "$ARGUMENTS"`
  max_seq_length: 2048
...
2. Save to: `config/lora-$ARGUMENTS.yaml`
3. Adjust hyperparameters for your dataset
