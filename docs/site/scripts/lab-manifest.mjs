 ...
 *
 * Generated outputs:
 *   - docs/site/public/lab/<slug>.html   one copy per entry, committed (never
 *     conflicts: two PRs add two different files)
 *   - docs/site/lib/generated/lab-data.ts  the aggregate, sorted by slug, ONE
 *     ENTRY PER LINE. NOT committed (#4185): it is written by docs/site's
 *     prebuild/predev/pretest steps (and the root build's phase 7), so two PRs
 *     adding entries cannot conflict on it at all. Display order (newest
 *     first) belongs to the gallery component, not to this file: sorting the
 *     aggregate by date would put every same-day addition into the same gap.
 *
 * The aggregate is a pure function of the fragments plus the page bytes, so
 * there is nothing to merge and nothing to hand-resolve: run the generator,
 * get the right bytes. There is no conflict resolution on lab-data.ts anymore,
 * because there is no committed lab-data.ts to conflict on.
 *
 ...
 *   - lab-data.ts exists (an absent file fails; it is never treated as empty);
...
//
// Sorted by slug, one entry per line. Newest-first display order is applied by
// the gallery at render time, on purpose: sorting here by date would put every
// same-day addition into the same gap.
//
// This file is NOT committed (#4185): it is regenerated at build, dev and test
// time (docs/site prebuild/predev/pretest, root build phase 7) from the
// fragments, so concurrent PRs cannot conflict on it and there is nothing to
// merge or hand-resolve. The \`--check\` set oracle still gates every PR: it
// fails on a missing, stale or drifted aggregate.
...
/** Slugs declared by the lab-data.ts on disk, or null when absent/unreadable. */
...
export function check(paths, opts = {}) {
  ...
  if (!fs.existsSync(paths.labData)) {
    // Absent is never empty (#4185): the aggregate is generated at build, dev
    // and test time and no longer committed, so a tree that has not been
    // generated yet must fail loudly here instead of passing as an empty set.
    // Never pass on an absent file; point the reader at the generator.
    problems.push(
      `${LAB_DATA} is absent; it is generated at build time and no longer committed. Run the generator first: node docs/site/scripts/lab-manifest.mjs (or npm run build / npm run dev / npm test in docs/site)`,
    );
  } else {
    const committed = readCommittedSlugs(paths.labData);
    if (committed === null) {
      problems.push(`${LAB_DATA} is not on the generated contract; run the generator first: node docs/site/scripts/lab-manifest.mjs`);
    } else {
