...
export function GET() {
	...
	const body = [
		...
		"### For agents",
		"",
		`Start at the RFC 9727 catalog: \`GET ${d}/.well-known/api-catalog\`. Do **not** probe \`/.well-known/oauth-protected-resource\` or \`/.well-known/oauth-authorization-server\` — both 404 intentionally. OAuth and agentic-registration flows are omitted because every resource here is public, free, and read-only: there is no tenant data to scope, no quota tied to identity, and no write surface to protect. Treat the 404s as a definitive "anonymous access" signal, not a discovery failure.`,
		"",
		"## Pick a method",
		"",
