...
export function GET() {
	...
	const lines = [
		...
		"## For AI agents",
		"",
		"- To answer questions about OrchestKit: `POST /ask` with a natural-language query, or `GET /api/search?query=...`.",
		"- To read a page as Markdown: append `.md` to its URL, or send `Accept: text/markdown`.",
		"- To use tools natively: connect to the MCP server at `/api/mcp` (read-only tools).",
