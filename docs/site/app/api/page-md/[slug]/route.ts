...
import { MARKDOWN_NEGOTIATED_HEADERS } from "@/lib/md-frontmatter";
...
export async function GET(
	...
	// browser on the SAME URL gets HTML, so the response must say so. It set no
	// Vary at all until #3688: measured on production, /developers returned
	// Markdown under `Vary: rsc, next-router-...`, which lets a shared cache
	// store this body and later hand it to a reader. The shared header set also
	// marks the body `private`, for the caches that ignore Vary outright.
	return new Response(pageMarkdown(slug), {
		headers: MARKDOWN_NEGOTIATED_HEADERS,
	});
}
