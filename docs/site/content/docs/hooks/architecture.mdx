...
OrchestKit compiles 171 hooks into **11 event-specific bundles** using esbuild. When Claude Code fires a hook, only the bundle for that event type is loaded -- not the entire codebase.
...
| `prompt.mjs` | 57 KB | 13 | UserPromptSubmit | unified-dispatcher (9 consolidated hooks), profile-injector, memory-context |
...
| `stop.mjs` | 33 KB | 12 | Stop | unified-stop-dispatcher (fans out to 7 hooks internally) |
...
**Used by:** 7 fire-and-forget dispatchers (see below).
...
A specialized pattern for the Stop event. Instead of passing input via command-line arguments, it writes a work file to disk and spawns a long-lived background worker. This is necessary because Stop hooks run 7 functions in parallel and may take several seconds.
...
| `prompt/memory-context` | UserPromptSubmit | 1 | Searches graph for entities matching prompt keywords |
...
| `stop-fire-and-forget.mjs` | Stop | 7 | All stop hooks via background worker (see below) |
...
The Stop event has the most complex execution pipeline because it must run 7 cleanup hooks without blocking session exit.
...
<Tabs items={["Global (42)", "Agent-Scoped (44)", "Skill-Scoped (19)"]}>
<Tab value="Global (42)">
...
The 45 agent-scoped hooks use 5 underlying hook functions from the `agent.mjs` bundle: `block-writes`, `ci-safety-check`, `deployment-safety-check`, `migration-safety-check`, and `security-command-audit`. Multiple agents may reference the same hook function.

</Tab>
<Tab value="Skill-Scoped (19)">
...
In addition to `"type": "command"` hooks that spawn a shell process, OrchestKit registers 12 hooks with `"type": "http"`. These hooks POST the hook input as JSON directly to a URL, bypassing the Node.js process spawn entirely.

```json
{
  "type": "http",
  "url": "http://localhost:${ORCHESTKIT_PORT}/hooks/analytics/session-event",
  "statusMessage": "Syncing analytics..."
}
```

HTTP hooks are used for analytics, webhook integrations, and external service notifications where the overhead of spawning a Node.js process is unnecessary. They follow the same input/output contract -- the URL receives a JSON POST body matching `HookInput` and returns a JSON response matching `HookResult`.

## The once:true Pattern (CC 2.1.69+)

Hooks registered with `"once": true` auto-remove after their first successful fire in a session. OrchestKit uses this for 13 skill context loaders that inject skill-specific context (guidelines, templates, checklists) on the first matching event and then unregister themselves to avoid redundant work on subsequent fires.

```json
{
  "type": "command",
  "command": "node ${CLAUDE_PLUGIN_ROOT}/hooks/bin/run-hook.mjs prompt/memory-context-loader",
  "once": true
}
```

This pattern is ideal for one-time initialization: context injection, environment detection, or consent checks that only need to run once per session.

## statusMessage for Spinner UX (CC 2.1.69+)

Hooks can declare a `"statusMessage"` string in their `hooks.json` registration. When present, Claude Code displays the message as a spinner label while the hook executes, giving the user feedback that work is happening.

```json
{
  "type": "command",
  "command": "node ${CLAUDE_PLUGIN_ROOT}/hooks/bin/run-hook.mjs lifecycle/session-context-loader",
  "statusMessage": "Loading session context..."
}
```

OrchestKit uses `statusMessage` on 15 hooks -- primarily context loaders, PR enrichers, and setup hooks where execution takes more than a few hundred milliseconds. For fast hooks (under 50ms), omit `statusMessage` to avoid spinner flicker.

## Hook Overrides

Hooks can be disabled only at user scope by creating `~/.claude/hook-overrides.json` (a project `.claude/hook-overrides.json` is ignored; AF-12 / #4220):
...
`run-hook.mjs` reads this file from the user home before executing any hook. If the hook name appears in the `disabled` array, it returns silent success without loading the bundle. Security hooks in `SECURITY_HOOKS` cannot be disabled this way.
