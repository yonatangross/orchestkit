...
description: "How OrchestKit's 95-hook system works: bundles, dispatchers, execution modes, and the stop pipeline."
---
...
OrchestKit compiles 95 hooks into **12 event-specific bundles** using esbuild. When Claude Code fires a hook, only the bundle for that event type is loaded -- not the entire codebase.
