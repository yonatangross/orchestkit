---
title: "105 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 42 | Fire on every matching event regardless of agent (10 matchers, 20 dispatchers, 12 native HTTP) |
| **Agent-scoped hooks** | 44 | Fire only when a specific agent is active |
| **Skill-scoped hooks** | 19 | Fire only during specific skill operations |
| **Total** | **101** | Registered in `hooks.json`, compiled into 12 split bundles |

All 101 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
...
| `skill.mjs` | 52 KB | 19 | Skill operations |
...
|------|----------|---------|
| `"command"` | Spawn a shell command, pipe JSON via stdin/stdout | All blocking and fire-and-forget hooks |
...
|------|---------|---------|
| `"once": true` | Run only on the first matching event, then auto-remove (CC 2.1.69+) | 13 skill context loaders that inject context on first fire only |
