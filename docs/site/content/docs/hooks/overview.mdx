---
title: "95 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 34 | Fire on every matching event regardless of agent (13 matchers, 16 dispatchers, 7 native async) |
| **Agent-scoped hooks** | 54 | Fire only when a specific agent is active |
| **Skill-scoped hooks** | 7 | Fire only during specific skill operations |
| **Total** | **95** | Registered in `hooks.json`, compiled into 12 split bundles |

All 95 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
