---
title: "173 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 106 | Fire on every matching event regardless of agent |
| **Agent-scoped hooks** | 45 | Fire only when a specific agent is active |
| **Skill-scoped hooks** | 22 | Fire only during specific skill operations |
| **Total** | **173** | Registered in `hooks.json`, compiled into 12 split bundles |

All 173 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
