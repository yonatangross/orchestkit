...
|----------|-------|-------------|
| **Global hooks** | 151 | Fire on every matching event regardless of agent |
...
| **Total** | **219** | Registered in `hooks.json`, compiled into 12 split bundles |

All 219 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
