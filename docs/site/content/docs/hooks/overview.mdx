---
title: "119 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 91 | Fire on every matching event regardless of agent |
| **Agent-scoped hooks** | 22 | Fire only when a specific agent is active |
| **Skill-scoped hooks** | 6 | Fire only during specific skill operations |
| **Total** | **119** | Registered in `hooks.json`, compiled into 11 split bundles |

All 119 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
...
9 hooks handle first-run setup, maintenance checks, mem0 backup configuration, monorepo detection, and analytics dashboard initialization.
</Step>
...
5 hooks fire: a unified dispatcher (fire-and-forget) launches 7 background tasks (mem0 retrieval, pattern sync, coordination init), the session context loader reads prior state, the analytics consent checker runs, the PR status enricher loads context for `--from-pr` sessions, and the prefill guard scans for deprecated API patterns.
</Step>
...
- **SubagentStart** (7 hooks): Context stager, graph memory inject, mem0 memory inject, subagent validator, context gate, task linker, model cost advisor.
...
OrchestKit does not load all 119 hooks into memory for every event. Instead, esbuild compiles hooks into **11 event-specific bundles** plus 1 unified bundle for CLI tools.
