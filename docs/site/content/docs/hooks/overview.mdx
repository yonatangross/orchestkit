---
title: "104 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 36 | Fire on every matching event regardless of agent |
| **Agent-scoped hooks** | 47 | Fire only when a specific agent is active |
| **Skill-scoped hooks** | 21 | Fire only during specific skill operations |
| **Total** | **104** | Registered in `hooks.json`, compiled into 12 split bundles |

All 104 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
