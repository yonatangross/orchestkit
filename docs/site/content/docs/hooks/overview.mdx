---
title: "174 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 153 | Fire on every matching event regardless of agent |
| **Agent-scoped hooks** | 0 | Fire only when a specific agent is active |
| **Skill-scoped hooks** | 22 | Fire only during specific skill operations |
| **Total** | **174** | Registered in `hooks.json`, compiled into 11 split bundles |

All 174 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
