---
title: "122 Hooks: What Fires When"
...
|----------|-------|-------------|
| **Global hooks** | 94 | Fire on every matching event regardless of agent |
...
| **Total** | **122** | Registered in `hooks.json`, compiled into 11 split bundles |

All 122 hooks are written in TypeScript, compiled via esbuild into ESM bundles, and executed by Node.js. Zero external dependencies in production.
...
OrchestKit does not load all 122 hooks into memory for every event. Instead, esbuild compiles hooks into **11 event-specific bundles** plus 1 unified bundle for CLI tools.
