...
```python
# Constant-time COMPARISON only. The SHA-256 hashing below is NOT safe password
# storage: unsalted and single-round, so it is trivially cracked by rainbow tables
# and GPU brute force. The only lesson here is compare_digest instead of ==.
import hmac
import hashlib

def compare_hashes_demo(stored_hash: str, provided_password: str) -> bool:
    # UNSAFE hashing, shown only to give compare_digest something to compare
    provided_hash = hashlib.sha256(provided_password.encode()).hexdigest()

    # SAFE: hmac.compare_digest uses constant-time comparison
    return hmac.compare_digest(stored_hash, provided_hash)

# Correct password storage: a salted, memory-hard KDF via argon2-cffi
# (pip install argon2-cffi). Do not use passlib, it was last released in 2020
# and breaks on Python 3.13+ where PEP 594 removed the stdlib crypt module.
from argon2 import PasswordHasher
from argon2.exceptions import VerifyMismatchError
...
def hash_password(plain_password: str) -> str:
    # SAFE: per-password salt and cost parameters are embedded in the output
    return ph.hash(plain_password)

def verify_password(plain_password: str, hashed_password: str) -> bool:
    # SAFE: Argon2 handles salting and timing-safe comparison internally
    try:
