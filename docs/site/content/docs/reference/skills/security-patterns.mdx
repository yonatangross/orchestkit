...
### Scan dependencies for known CVEs to prevent critical vulnerabilities reaching production — HIGH
...
### Run static analysis with Semgrep to catch injection and insecure patterns before production — HIGH
...
### Detect committed secrets in git history and enforce immediate credential rotation — CRITICAL
