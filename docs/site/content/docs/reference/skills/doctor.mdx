...
```
Installed Plugins: 1
- ork: 92 skills, 33 agents, 101 hook entries
```
...
OrchestKit requires Claude Code >= 2.1.75. This matrix documents which CC features OrchestKit depends on and their minimum version requirements.
...
| Skill-file deadlock fix on `git pull` | 2.1.73 | Fixes hang with 92-skill plugin after `git pull` | CC hangs during/after git pull |
...
| No-op system reminder injection fix | 2.1.73 | ~2K tokens/turn recovered across 101 hooks | Empty system-reminder tags waste context |
