...
| `/output-style` deprecated → `/config` | 2.1.73 | No OrchestKit references to /output-style | None, already using /config. Superseded in 2.1.269: `/output-style [name]` was restored to list and switch output styles. |
...
| Default effort `high` | 2.1.94 | Default effort level is `high` for API key, Bedrock, Vertex, Foundry, Team and Enterprise users (use `/effort low` or `/effort medium` to reduce) | Default `medium`, token budgets scaled conservatively |
