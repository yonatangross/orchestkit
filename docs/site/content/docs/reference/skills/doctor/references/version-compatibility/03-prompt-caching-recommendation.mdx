...
| v10.0.0-alpha.8x | 2.1.251 | **CC 2.1.269 to 2.1.272 adoption (2026-09-15): `latest_known` 2.1.263 to 2.1.272, `supported_floor` STAYS 2.1.251 by the manual override. Four releases triaged into four matrix rows: 2.1.269 adds plugin eval, restored named output style selection, Bash edit diffs and six documented settings or environment controls; 2.1.270 is a read only git permission regression fix; 2.1.271 adds agent instruction omission, auto mode inline shell permission rules, classifier reviewed subagent hand backs, deadline bound Monitor watches, hook wait feedback and a plugin install accept command path folded into GH-4054; 2.1.272 has bug fixes only. The `/output-style` 2.1.73 deprecation row is superseded. No ork code change; design items tracked in GH-4135.** |
