...
**Version Compatibility**, page 4 of 4:

1. [Overview](/docs/reference/skills/doctor/references/version-compatibility/00-overview)
...
4. **Prompt Caching Recommendation** (this page)
...
| >= 2.1.277 | **Minimum (current floor)** | **OrchestKit's supported floor. Anything below is unsupported. `shared/cc-support.json` is the single source of truth; this row mirrors it.** |

## Host Support Levels

Non-CC hosts. Each row is a measured verdict doctor can name; evidence lives in
the dated audits under `docs/audits/` (`multi-engine-support-2026-09-08.md` for
pi, Codex and Cursor; `agy-host-support-2026-09-18.md` for agy 1.2.6).

| Host | Support level | Loads | Does not load | Evidence |
|------|---------------|-------|---------------|----------|
| Claude Code | Full | skills, agents, hooks, `.mcp.json`, `CLAUDE.md` | | shipped |
| Cursor | Full minus hooks | skills, agents, plugin `rules` | Claude hook scripts | shipped |
| Codex | Portable pack | `ork-codex` skills + role templates, `$ork-<skill>` | hooks | `multi-engine-support-2026-09-08.md` |
| pi | Skills via manifest | `pi install` skills, `AGENTS.md` | hooks | `multi-engine-support-2026-09-08.md` |
| Muse Code | Skills via `.agents/skills` | skills.sh starter SKILL.md | hooks (`.muse/hooks.json` is its own) | host page |
| OpenCode | Skills via `.agents/skills` | skills.sh starter SKILL.md | hooks | `multi-engine-support-2026-09-08.md` |
| Devin | Skills via git plugin | 76 of 107 skills, `AGENTS.md` | hooks (`hooks.v1.json` contract), rules | GH-4146, host page |
| Antigravity (`agy`) | Skills via workspace `.agents/skills` | workspace `.agents/skills`, `.agents/skills.json` entries, plugin `skills/`+`agents/`+`commands/` (commands convert to skills, `ork:<name>` namespace via `agy plugin install`), workspace `.agents/hooks.json` in agy's own schema, global + plugin `mcp_config.json`, `AGENTS.md` per directory | `~/.agents/skills` (the skills.sh `-g` target), repo `.mcp.json`, ork's CC-schema `hooks/hooks.json`, `.cursor-plugin/commands/*.md` wrappers | `agy-host-support-2026-09-18.md` (agy 1.2.6, marker-verified) |
...
```
Claude Code: 2.1.277 (OK)
- Minimum required: 2.1.277
```
...
|-----------|--------|-------------|
| v10.0.0-beta.7x | 2.1.277 | **CC 2.1.278 + 2.1.280 adoption and Opus 5.5 (2026-09-22): `latest_known` 2.1.277 to 2.1.280, `supported_floor` STAYS 2.1.277. 2.1.279 was never published. 2.1.278 moves auto mode to the server-side classifier by default (`CLAUDE_CODE_AUTO_MODE_SERVER=0` opts out). 2.1.280 makes Claude Opus 5.5 (`claude-opus-5-5`, $4/$20) the default Opus: the `opus` alias advances in `models.vocab.json`, the row is priced (it had fallen to Sonnet rates), and the model-recency gate's date-snapshot exemption is fixed (it read `claude-opus-5` as current because `claude-opus-5-5` extends it with a hyphen). Also named: `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`, and the agent-type `PermissionRequest` removal (no-op, all four ork hooks are command hooks).** |
| v10.0.0-beta.5x | 2.1.251 | **CC 2.1.273 to 2.1.277 adoption (2026-09-18): `latest_known` 2.1.272 to 2.1.277, `supported_floor` STAYS 2.1.251 by the manual override. Five releases triaged into five matrix rows (`shared/cc-adoption-gaps.json`, `docs/audits/cc-adoption-2.1.273-277-triage-2026-09-18.md`): 2.1.273 hardens the permission checker (blockReadsOutsideWorkingDirectories prompt skips, subshell `rm` hiding), stops lost subagent results and wrong-worktree scheduled tasks, and reverts part of the 2.1.268 deny-rule change; 2.1.274 fixes MCP transports (SSE-only servers, per-server timeouts, listChanged refresh), closes a worktree-isolated Bash expansion escape and adds `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`; 2.1.275 syncs claude.ai skills and plugins into terminal sessions (`syncClaudeAiSkills`/`syncClaudeAiPlugins` opt-outs), stops marketplace URL credential leaks and npm plugin install scripts; 2.1.276 fixes the 2.1.275 ANTHROPIC_BASE_URL regression; 2.1.277 lands the built-in agents-md module (AGENTS.md read when no CLAUDE.md sits on the walk, four `instructionFiles` values, the `/config` "Project instructions" toggle, verified by the codeword canary at `tests/canary/agents-md-fallback.sh`), the five-tier 92 plus 33 event engine, removes the deprecated TaskOutput tool, and fixes `--worktree` project-skill loading plus the `sandbox.excludedCommands` compound exemption. `claude plugin test` verified absent on 2.1.277. No ork runtime change.** |
...
| v10.0.0-alpha.8x | 2.1.251 | **CC 2.1.269 to 2.1.272 adoption (2026-09-15): `latest_known` 2.1.263 to 2.1.272, `supported_floor` STAYS 2.1.251 by the manual override. Four releases triaged into four matrix rows: 2.1.269 adds plugin eval, restored named output style selection, Bash edit diffs and five documented settings or environment controls; 2.1.270 is a read only git permission regression fix; 2.1.271 adds agent instruction omission, auto mode inline shell permission rules, classifier reviewed subagent hand backs, deadline bound Monitor watches, hook wait feedback, per command `allowed_domains` and a plugin install accept command path folded into GH-4054; 2.1.272 has bug fixes only. The `/output-style` 2.1.73 deprecation row is superseded. No ork code change; design items tracked in GH-4135.** |
