...
This reference is split across 7 pages so each stays under the 25K-token page budget. The rules and reference material that used to sit at the bottom of this page now live at:

- [Rules](/docs/reference/skills/configure/rules): 2 rules for the Configure skill: Backup existing configuration before making changes to enable safe rollback; Validate configuration after writing to catch malformed JSON and invalid settings
- [References: CC Version-Specific Settings](/docs/reference/skills/configure/references/cc-version-settings): Configure CC 2.1.7-specific features
- [References: HTTP Hooks and Streaming Telemetry (CC 2.1.63+)](/docs/reference/skills/configure/references/http-hooks): OrchestKit used to emit hook events over two channels that POSTed to the same downstream endpoint, so every event was delivered twice
- [References: MCP Configuration](/docs/reference/skills/configure/references/mcp-config): MCPs (Model Context Protocol servers) enhance OrchestKit commands but are NOT required.
- [References: Operator-Scope Settings](/docs/reference/skills/configure/references/operator-scope-settings): Paste-ready JSON for controls a plugin bundle cannot carry.
- [References: Preset Definitions](/docs/reference/skills/configure/references/presets): Everything enabled - full AI-assisted development.
