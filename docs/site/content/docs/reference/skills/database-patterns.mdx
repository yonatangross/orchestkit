...
def run_migrations_online() -> None:
    # Guard: if a greenlet/event loop already exists (e.g. FastAPI lifespan),
    # use await_only() instead of asyncio.run() to avoid "loop already running".
    if in_greenlet():
...
- Using bare `asyncio.run()` when a greenlet/event loop already exists (e.g. FastAPI lifespan); use the `in_greenlet()` guard with `await_only()` as shown above
