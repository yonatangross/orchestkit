...
> **Brainstorm** Design exploration using parallel agents through a 7-phase process: topic analysis, memory context, divergent ideation (10+ ideas), feasibility filtering, evaluation with devil's advocate scoring (0-10 across 7 dimensions), synthesis of top approaches, and trade-off comparison. Supports open exploration, constrained design, comparison, quick ideation, and iterative optimization modes. Use when brainstorming ideas, exploring solutions, or comparing alternatives.
...
This reference is split across 5 pages so each stays under the 25K-token page budget. The rules and reference material that used to sit at the bottom of this page now live at:

- [Rules](/docs/reference/skills/brainstorm/rules): 4 rules for the Brainstorm skill: Limit parallel brainstorm agents to 5 maximum to avoid diminishing returns and context waste; Flag ideas that exceed current team or infrastructure capacity using tier-based complexity ceilings; Brainstorm must converge to actionable recommendations with clear next steps, not just list ideas; Each idea must have a feasibility score (1-5) before synthesis; flag unfeasible ideas explicitly
- [References](/docs/reference/skills/brainstorm/references): 12 references for the Brainstorm skill: Claude Code; Common Pitfalls; Devils Advocate Prompts; Divergent Techniques; Effort Scaling; Evaluation Rubric; Example Session Auth; Example Session Dashboard; Iterative Optimization Mode; Mcp Probe Resume; Phase Workflow; Socratic Questions
- [Checklists](/docs/reference/skills/brainstorm/checklists): 2 checklists for the Brainstorm skill: Brainstorm Completion; Brainstorm Session Checklist
- [Examples](/docs/reference/skills/brainstorm/examples): 1 examples for the Brainstorm skill: Orchestkit Feature Brainstorm
