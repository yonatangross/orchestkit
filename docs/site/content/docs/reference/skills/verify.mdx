...
Pass `MODEL_OVERRIDE` to all Agent() calls via `model=MODEL_OVERRIDE` when set. Accepts symbolic names (`opus`, `sonnet`, `haiku`) or full IDs (`claude-opus-4-6`) per CC 2.1.75.
...
```json
{
  ...
  "evaluation": [
    ...
    {"severity": "warning", "message": "CONTENT: Stats show '79 skills' but should be '92 skills' — stale count detected"},
