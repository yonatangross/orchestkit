...
> Claude Design handoff orchestration: parses bundles exported from claude.ai/design, normalizes their schema, maps proposed components against the existing codebase via component-search before scaffolding, and tracks bundle-to-PR provenance so design intent stays linked to shipped code
