...
OrchestKit is a single plugin, **ork**: <Count k="skills" /> skills, <Count k="agents" /> agents, <Count k="hooks" /> hooks. Inside Claude Code run `/plugin marketplace add yonatangross/orchestkit`, then `/plugin install ork`. From a terminal: `claude plugin marketplace add yonatangross/orchestkit && claude plugin install ork@orchestkit`.
...
171 total: 152 global hooks, 0 agent-scoped hooks, and 21 skill-scoped hooks. They are compiled into 11 split bundles for faster loading.
