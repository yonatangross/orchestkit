...
There is no difference. OrchestKit is now a single unified plugin: **ork** — 69 skills, 38 agents, 78 hooks. The names `orkl` and `ork-creative` are preserved as backward-compatible install aliases that deliver the exact same plugin.
...
69 total: 29 command skills (user-invocable via `/ork:skillname`) and 40 reference skills (knowledge modules auto-injected by agents and hooks).
