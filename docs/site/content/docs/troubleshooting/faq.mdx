...
OrchestKit requires Claude Code 2.1.75 or later. Check your version:
...
OrchestKit is a single plugin: **ork** — 92 skills, 33 agents, 101 hooks. Install with `claude install orchestkit/ork`.
...
92 total: 19 command skills (user-invocable via `/ork:skillname`) and 73 reference skills (knowledge modules auto-injected by agents and hooks).
...
105 total: 42 global hooks (10 matchers, 20 dispatchers, 12 native HTTP), 44 agent-scoped hooks, and 19 skill-scoped hooks. They are compiled into 12 split bundles for faster loading.
