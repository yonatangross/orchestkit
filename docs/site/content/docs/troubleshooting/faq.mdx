...
OrchestKit is a single plugin: **ork** — 102 skills, 36 agents, 112 hooks. Install with `claude install orchestkit/ork`.
...
99 total: 21 command skills (user-invocable via `/ork:skillname`) and 79 reference skills (knowledge modules auto-injected by agents and hooks).
...
109 total: 40 global hooks, 47 agent-scoped hooks, and 22 skill-scoped hooks. They are compiled into 12 split bundles for faster loading.
