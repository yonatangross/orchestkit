...
|--------|--------|-------------|
| `orkl` | 88 | Universal toolkit -- works for any stack |
| `ork-creative` | 16 | Video production add-on -- demo recording, Remotion, storyboarding |
| `ork` | 199 | Full specialized -- includes orkl + creative + Python, React, LLM/RAG, LangGraph, MCP skills |

Both tiers include all 36 agents, 119 hooks, and all memory skills. The difference is only in specialized reference skills. If you work with Python, React, or LLM pipelines, use `ork`. If you want a lighter install for general development, use `orkl`.
...
199 total: 24 command skills (user-invocable via `/ork:skillname`) and 175 reference skills (knowledge modules auto-injected by agents and hooks).
...
119 total: 91 global hooks, 22 agent-scoped hooks, and 6 skill-scoped hooks. They are compiled into 11 split bundles for faster loading.
