...
OrchestKit is a single plugin: **ork** — 98 skills, 35 agents, 106 hooks. Install with `claude install orchestkit/ork`.
...
98 total: 20 command skills (user-invocable via `/ork:skillname`) and 78 reference skills (knowledge modules auto-injected by agents and hooks).
...
106 total: 37 global hooks, 47 agent-scoped hooks, and 22 skill-scoped hooks. They are compiled into 12 split bundles for faster loading.
