...
`/ork:implement` plans the feature, then runs parallel specialist agents for frontend, backend (if your app has API routes), and test generation inside an isolated git worktree, and verifies the result before handing it back.
