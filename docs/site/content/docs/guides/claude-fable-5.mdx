...
Fable 5's safety classifiers fall back to Opus 5 in a small fraction of sessions, most likely on cyber/bio-adjacent content. For security-audit workloads this means a fable-pinned auditor could silently switch models mid-run, which is one reason OrchestKit keeps its own security agents on an explicit `opus` pin rather than fable.
