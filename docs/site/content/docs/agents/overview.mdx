...
description: "30 specialized agents -- how they activate, what they know, and when to use each."
full: true
---

import { LazyAgentSelector as AgentSelector } from '@/components/lazy';

# 30 Specialists, One Prompt Away
...
- **`opus`** -- Used for complex reasoning tasks: architecture design, security auditing, system review, workflow design. 9 agents use Opus.
- **`sonnet`** -- Used for focused production tasks: demo video production. 1 agent uses Sonnet.
- **`inherit`** -- Uses whatever model the parent session is running. 20 agents inherit. This is the most flexible option and is recommended for most agents.
...
| `frontend-performance-engineer` | inherit | Core Web Vitals, bundle analysis, render optimization |
...
| `eval-runner` | inherit | LLM evaluation, structured eval datasets, quality metrics |
...
| `ui-feedback` | inherit | Processes UI annotations, maps element paths to source code |
