...
|--------|--------------------|
| Backend API | `api-design`, `python-backend`, `async-jobs` |
| Database | `database-patterns`, `distributed-systems` |
| Frontend | `react-server-components-framework`, `ui-components`, `zustand-patterns`, `responsive-patterns` |
| Security | `security-patterns`, `mcp-patterns` |
| Testing | `testing-unit`, `testing-e2e`, `testing-integration`, `golden-dataset` |
| DevOps | `devops-deployment`, `github-operations`, `monitoring-observability` |
| LLM / AI | `langgraph`, `llm-integration`, `rag-retrieval`, `multimodal-llm` |
| Architecture | `architecture-decision-record`, `architecture-patterns`, `domain-driven-design` |
...
- **9 agents** use `opus`: `backend-system-architect`, `security-auditor`, `ai-safety-auditor`, `security-layer-auditor`, `event-driven-architect`, `workflow-architect`, `infrastructure-architect`, `python-performance-engineer`, `system-design-reviewer`
- **1 agent** uses `sonnet`: `demo-producer`
- **20 agents** use `inherit`
...
```markdown
## Integration
- **Receives from:** product-strategist (requirements), workflow-architect (architecture)
...
- [Agents Overview](/docs/agents/overview) -- See all 33 existing agents for inspiration
