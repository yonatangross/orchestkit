...
OrchestKit solves this with a **4-tier memory architecture** that stores decisions once and surfaces them automatically in every future session. No manual copy-pasting. No re-explaining your stack. Decisions persist across conversations, projects, and machines.
...
| **3. Mem0** | Cloud API | `MEM0_API_KEY` env var | Semantic search across all sessions and projects. Optional. |
| **4. CC Native** | `~/.claude/projects/*/memory/MEMORY.md` | None (auto-sync) | Injected directly into Claude's system prompt. Survives plugin removal. |

<Callout type="info">
Tier 1 (Graph) and Tier 4 (CC Native) require zero configuration. They work the moment you install OrchestKit. Tier 2 adds local durability automatically. Tier 3 enables cross-project semantic search when you add a Mem0 API key.
</Callout>
...
/ork:memory search --mem0 "database patterns"
```

The `--mem0` flag returns graph results first, then cloud results, with duplicates removed and cross-references marked as `[CROSS-REF]`.

### Step 4: Configure Mem0 for Cloud Persistence (Optional)

For semantic search across all your projects and sessions, enable Tier 3 by setting a single environment variable:
...
Restart Claude Code. Now use the `--mem0` flag to write to both graph and cloud simultaneously:
...
```
Remembered (database): "Use connection pooling with min=5, max=20 for PostgreSQL"
   -> Stored in knowledge graph
   -> Created entity: connection-pooling (Pattern)
   -> Created entity: PostgreSQL (Technology)
   -> Created relation: PostgreSQL -> USES -> connection-pooling
   Graph: 2 entities, 1 relation
   -> Also synced to mem0 cloud
```

<Callout type="info">
Without `MEM0_API_KEY`, all `--mem0` commands gracefully fall back to graph-only storage. You see a notice: "mem0 sync requested but MEM0_API_KEY not configured (graph-only)". Nothing breaks.
</Callout>

### Step 5: Verify Memory System Health
...
If a tier shows an issue (for example, `Mem0 Cloud: disconnected (MEM0_API_KEY not set)`), the status output tells you exactly what to configure.
...
/ork:remember "JWT for auth"
    ...
    +---> Tier 4: CC Native MEMORY.md (if confidence >= 0.7)
```
...
- **[Memory Overview](/docs/memory/overview)** -- Deep dive into the 4-tier architecture
- **[Configuration](/docs/getting-started/configuration)** -- Set up environment variables including `MEM0_API_KEY`
