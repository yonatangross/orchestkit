...
| `session-summary` | Hook (stop) | At session end, suggests a graph entity capture and nudges `/ork:remember` on busy sessions |
...
`pretool/mcp/memory-validator` runs on `mcp__memory__*` PreToolUse. It denies oversized payloads and warns on missing required fields or unknown entity types before the call proceeds.

### On Session End

`stop/session-summary` runs when the session had meaningful work (20 or more tool calls). It returns an `additionalContext` suggestion to create a session graph entity, and for high-activity sessions (over 50 tool calls) it nudges you to run `/ork:remember`. It does not write local JSONL or MEMORY.md itself.

`stop/handoff-writer` may include recent entries from `.claude/logs/decisions.jsonl` in the handoff note when that file exists.
...
<Callout type="info">
**Store investigation context before ending a session.** Run `/ork:remember "Session summary: completed X, found Y, next steps Z"` before closing. `session-summary` only suggests graph capture via additionalContext; it does not persist MEMORY.md for you. Explicit `/ork:remember` (and optional filesystem memory edits) are the durable write path.
</Callout>
