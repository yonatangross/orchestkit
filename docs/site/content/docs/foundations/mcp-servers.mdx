...
- `orchestkit_install_plan`: Generate install and invocation commands for given skills or goals. Returns a plan; executes nothing.
- `orchestkit_doctor_check`: Validate a pasted config fragment against documented schemas. Reads only the input.

All three tools are read-only by construction. The install plan tool returns shell commands to run; it never runs them. The doctor tool validates pasted input against schemas; it touches no files.
