...
These agents intentionally have `mcpServers: []`, so they work with built-in tools only:
...
The servers above are ones OrchestKit *consumes*. OrchestKit also *publishes* one: the **OrchestKit Docs MCP** server exposes this documentation to any MCP client, read-only, with no authentication.

| Surface | URL |
|---------|-----|
| Streamable HTTP endpoint | `https://orchestkit.yonyon.ai/api/mcp` (POST; JSON-RPC 2.0) |
| Server card | [`/.well-known/mcp/server-card.json`](https://orchestkit.yonyon.ai/.well-known/mcp/server-card.json) |
| Local stdio image | `docker run -i --rm ghcr.io/yonatangross/orchestkit-docs-mcp` |
| Registry entry | `io.github.yonatangross/orchestkit` on [registry.modelcontextprotocol.io](https://registry.modelcontextprotocol.io/v0/servers?search=io.github.yonatangross/orchestkit) |

Two tools: `orchestkit_docs_search` (ranked pages with titles and URLs) and `orchestkit_docs_get` (one page as Markdown by path). The same API is described by the [OrchestKit OpenAPI 3.1 spec](https://orchestkit.yonyon.ai/openapi.json); every developer surface is indexed at the [OrchestKit developer resources hub](/developers).

## OrchestKit Actions MCP Server

OrchestKit also publishes a second MCP server: the **OrchestKit Actions MCP** exposes read-only CLI actions so agents can plan installations without executing anything. This is the product MCP that satisfies ora.ai's multi-surface coverage check.

| Surface | URL |
|---------|-----|
| Streamable HTTP endpoint | `https://orchestkit.yonyon.ai/api/mcp/actions` (POST; JSON-RPC 2.0) |
| Server card | [`/.well-known/mcp/actions-server-card.json`](https://orchestkit.yonyon.ai/.well-known/mcp/actions-server-card.json) |

Three tools:

- `orchestkit_list_skills`: Discover user-invocable skills with names, descriptions, and docs URLs.
- `orchestkit_install_plan`: Generate install and invocation commands for user-invocable skills or goals. Returns a plan; executes nothing.
- `orchestkit_doctor_check`: Run basic checks on a pasted JSON object config fragment. It does not parse YAML and reads only the input.

All three tools are read-only by construction. The install plan tool returns shell commands to run for user-invocable skills; it never runs them. The doctor tool performs basic checks on pasted JSON objects; it touches no files.

For hooks, the doctor tool expects a `hooks` object keyed by event name. Each event value must be an array of object entries, and each entry must contain a `hooks` array. It does not validate nested hook command fields.

## Sandbox and Dry-Run Mode

Every tool on the Actions MCP server is read-only. There is no sandbox mode to enable because nothing writes. The documentation states this honestly: no tool can mutate state, so no special `dry_run` flag exists.

If you need to test an installation without affecting your system, run the commands returned by `orchestkit_install_plan` in a disposable environment (a container, a VM, or a fresh git worktree). The MCP server itself cannot execute anything.
