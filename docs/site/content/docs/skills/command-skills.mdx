---
title: "17 Commands You Can Invoke"
...
Command skills are the 17 skills with `user-invocable: true` in their frontmatter. You trigger them by typing `/ork:skill-name` in Claude Code. Each one is a structured workflow that guides Claude through a multi-step process, often spawning parallel agents and pulling in reference skills automatically.
...
| `/ork:git-workflow` | Complete git workflow patterns including GitHub Flow branching, atomic commits, merge and rebase strategies, and recovery operations using reflog. | low | `/ork:git-workflow` |
...
| Command | Description | Complexity | Example |
|---------|-------------|------------|---------|
| `/ork:plan-viz` | Visualize planned changes before implementation. Compares before/after architecture, assesses risk, and analyzes execution order and impact. | low | `/ork:plan-viz` |
...
- [52 Reference Skills](/docs/skills/reference-skills) -- The knowledge library that command skills pull from.
