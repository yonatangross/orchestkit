...
Of OrchestKit's 92 skills, **19 are user-invocable** command skills (you type `/ork:skill-name`). Of those, **19 are listed** in Claude's prompt context — the other 2 use `disable-model-invocation: true` to save tokens but are still fully invocable via the `/ork:` prefix. The remaining 73 reference skills are hidden from the listing entirely:
