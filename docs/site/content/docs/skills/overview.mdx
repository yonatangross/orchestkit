...
Of OrchestKit's 71 skills, only **16 are listed** in Claude's prompt context. The remaining 55 are hidden via `disable-model-invocation: true` to save ~2,630 tokens per turn. Hidden skills are still fully functional:
