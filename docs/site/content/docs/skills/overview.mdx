...
Of OrchestKit's 79 skills, **18 are user-invocable** command skills (you type `/ork:skill-name`). Of those, **16 are listed** in Claude's prompt context — the other 2 use `disable-model-invocation: true` to save tokens but are still fully invocable via the `/ork:` prefix. The remaining 61 reference skills are hidden from the listing entirely:
