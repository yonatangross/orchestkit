...
- Technology knowledge not covered by OrchestKit's 89 built-in skills.
...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      command: "${CLAUDE_PLUGIN_ROOT}/hooks/bin/run-hook.mjs skill/my-validator"
```
...
For command skills that offer mode selection, use `AskUserQuestion` with `preview` panes. This gives users a side-by-side view of what each option does:
    ...
    "options": [
      {
        ...
        "description": "All dimensions scored",
        "preview": "```\nFull Analysis\n────────────\n  Phase 1: Gather context\n  Phase 2: Score dimensions\n  Phase 3: Generate report\n  Agents: 4 parallel\n```"
      },
      {
        "label": "Quick check",
        "description": "Single-pass brief output",
        "preview": "```\nQuick Check\n───────────\n  Single pass, 2 min\n  Score + top 3 issues\n  No agent spawning\n```"
      }
    ],
    "multiSelect": false
...
<Callout type="warn">
The `AskUserQuestion` schema is strict and a violation makes the picker silently fail to render (it looks "stuck"). Stay inside these limits:

- **2-4 options per question** (`minItems: 2`, `maxItems: 4`) — need more? split into multiple questions or merge options.
- **1-4 questions per call.**
- **Option fields are exactly `label`, `description`, `preview`** (`additionalProperties: false`). The preview field is `preview`, never `markdown` — `markdown` is rejected.
- **`preview` renders on single-select only** — do not add it to a `multiSelect: true` question.
- **No free-text questions** (min 2 options) — ask in plain prose instead.

`tests/skills/structure/test-askuserquestion-schema.sh` (in `npm run test:skills`) enforces all of these.
</Callout>

Use `multiSelect: true` when options can be combined (e.g., "which checks to run?" where a user might want both tests and security). Keep `multiSelect: false` for mutually exclusive modes (e.g., "full" vs "quick").

### Offer Plan Mode for Complex Skills

For skills that modify code, offer a "Plan first" option that uses `EnterPlanMode`:

```python
{"label": "Plan first", "description": "Research in plan mode before coding"}
```

When selected, call `EnterPlanMode`, do read-only research (Read, Grep, Glob), then `ExitPlanMode` with a plan for user approval before implementing.
...
Edit `manifests/ork.json`. Since OrchestKit uses `"skills": "all"`, any skill in `src/skills/` is automatically included. You only need to explicitly list skills if a manifest uses selective inclusion.
...
- The skill count matches the expected total (currently 79).
...
- [61 Reference Skills](/docs/skills/reference-skills) -- See how existing reference skills are structured.
- [23 Command Skills](/docs/skills/command-skills) -- If you want to create a command skill workflow.
