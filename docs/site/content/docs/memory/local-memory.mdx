...
Local memory is Tier 2 -- the **durability layer** that ensures no decision is lost even if graph operations fail, sessions end unexpectedly, or Mem0 cloud is unreachable. It uses JSONL (JSON Lines) files in the `.claude/memory/` directory, which is auto-created on first write.
...
.claude/memory/
  ...
  mem0-queue.jsonl       # Pending Mem0 cloud sync operations
...
Each file serves a distinct purpose. The three JSONL files form a **write-ahead queue** system: decisions land in the local files first, then are processed asynchronously into graph and cloud storage.
...
## `mem0-queue.jsonl` -- Pending Cloud Sync

When `MEM0_API_KEY` is configured, the memory writer also queues decisions for Mem0 cloud sync. This queue is processed at session end by the `mem0-queue-sync` hook.

### Record Structure

```json
{
  "text": "decision: Use cursor-based pagination for all list endpoints because Better performance for large datasets",
  "user_id": "orchestkit-decisions",
  "metadata": {
    "type": "decision",
    "category": "pagination",
    "confidence": 0.85,
  ...
  "queued_at": "2026-02-06T10:30:00.002Z"
}
```

### Processing at Session End

The `mem0-queue-sync` hook fires on the `stop` lifecycle event. It:

1. Checks if `MEM0_API_KEY` is configured (skips entirely if not)
2. Reads all queued memories from `mem0-queue.jsonl`
3. Deduplicates entries with the same text (keeps the most recent)
4. Groups entries by `user_id` for organized output
5. Generates CLI script commands for Claude to execute
6. Clears the queue file

If there are 5 queued memories across 2 scopes, the hook outputs:
...
### Scope: orchestkit-decisions

**Memory 1:**
python3 ${CLAUDE_PLUGIN_ROOT}/src/skills/mem0-memory/scripts/crud/add-memory.py \
  --text "decision: Use cursor-based pagination..." \
  --user-id "orchestkit-decisions" \
  --metadata '{"type":"decision","category":"pagination",...}'

**Summary:** 5 memories to sync across 2 scope(s).
Categories: pagination, authentication, database
```

<Callout type="info">
The mem0 queue is only populated when `MEM0_API_KEY` is configured. Without it, the queue file either does not exist or remains empty. No cloud operations are attempted.
</Callout>
...
1. **Writes always succeed locally.** The `appendToJsonl` function creates the directory if it does not exist and appends the record. Even if graph MCP is unreachable or Mem0 API is down, the decision is safely stored.

2. **Queues are processed when services become available.** Graph operations queue in `graph-queue.jsonl` until the MCP server processes them. Mem0 operations queue in `mem0-queue.jsonl` until session end sync.
...
| `mem0-queue.jsonl` has 50+ entries | `degraded` | Cloud sync is backed up; check if `MEM0_API_KEY` is valid |
...
memory-writer.ts
    ...
    +---> mem0-queue.jsonl   (only if MEM0_API_KEY set)
    |
    v
Hook processing
    |
    +---> graph-queue processed --> mcp__memory__create_entities
    +---> mem0-queue processed  --> python3 add-memory.py (at session end)
```
...
- `mem0-queue.jsonl` is cleared at session end by the `mem0-queue-sync` hook
...
- [Mem0 Cloud](/docs/memory/mem0-cloud) -- how the mem0 queue is processed for cloud persistence
- [Architecture Overview](/docs/memory/overview) -- the full 4-tier system
