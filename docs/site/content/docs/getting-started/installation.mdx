...
| **Codex** | Six workflow skills plus context7 (`ork-codex`) | `codex plugin add ork-codex@orchestkit-codex` after adding the marketplace |
| **pi** | Skills via the shipped `pi` manifest | `pi install git:github.com/yonatangross/orchestkit`, see [the engine guide](/docs/guides/orchestkit-on-pi-codex-cursor) |
...
That installs the six-skill `ork-codex` pack plus context7. For unattended
mechanical runs, add the shipped profile:
...
<Callout type="warn">
`ref main` is a cached snapshot, not a tracker. On the 2026-09-08 audit machine
`codex plugin list` reported `10.0.0-beta.5` while main was three releases ahead.
Run `codex plugin update` after an OrchestKit release, then compare
`codex plugin list | grep ork-codex` against
`jq -r .version plugins/ork-codex/.codex-plugin/plugin.json`.
</Callout>

The profile is a file at `~/.codex/ork-mech.config.toml`, never a
`[profiles.ork-mech]` table inside `config.toml`; codex rejects `--profile` while
that legacy table exists. Inside a git worktree add
`--add-dir "$(git rev-parse --path-format=absolute --git-common-dir)"` or the
first commit fails on `index.lock`. Details and the measurements are in
[the engine guide](/docs/guides/orchestkit-on-pi-codex-cursor).
