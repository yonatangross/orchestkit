...
The `#plugins/ork` fragment is what makes the install work: OrchestKit's skills live in the built plugin tree, and without the fragment Devin indexes the repository root and sees only `AGENTS.md`. The command installs to your personal scope, so the plugin follows you to cloud sessions and your other signed-in devices. Add `--local` to keep it on this machine only.
...
| Surface | Status |
|:-|:-|
| Skills | 76 of <Count k="skills" /> listed |
...
- OrchestKit's hooks are Claude Code `hooks.json` format. Devin expects `hooks.v1.json` with its own tool names, so hooks stay off until a Devin manifest ships.

<Callout type="warn">
After installing, run `devin plugins info ork`. If the skill list is empty you installed the repository root; reinstall with the `#plugins/ork` fragment.
</Callout>
