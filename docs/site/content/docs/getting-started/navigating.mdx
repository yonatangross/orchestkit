...
There are <Count k="commands" /> command skills. [Commands You Can Invoke](/docs/skills/command-skills) lists every one, grouped by task, with what it does and the arguments it takes. That page is generated from each skill's frontmatter, so it always matches the plugin you installed.
