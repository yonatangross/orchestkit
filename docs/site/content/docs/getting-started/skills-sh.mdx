...
Use skills.sh when the host is not Claude Code, Cursor's `ork` marketplace plugin, Codex's `ork-codex` pack, Pi's shipped `pi` manifest, or [Muse Code](/docs/getting-started/muse). That includes **OpenCode**, **Antigravity**, and other skills.sh clients.
...
**Antigravity** (`agy`, Google) has its own page: [Antigravity](/docs/getting-started/agy). Same skills.sh starter, loaded from workspace `.agents/skills`. One caveat: `skills add -g` lands in `~/.agents/skills`, which agy does not read.
