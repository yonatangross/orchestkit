...
| **Devin** | Plugin from git via `#plugins/ork`. 76 skills list; hooks and rules do not load | [Devin](/docs/getting-started/devin) |
| **Antigravity** | Skills via workspace `.agents/skills`. ork hooks and `.mcp.json` do not load | [Antigravity](/docs/getting-started/agy) |
...
<Cards>
  ...
  <Card title="Devin" description="CLI and Desktop plugin install from git. Skills load; hooks do not." href="/docs/getting-started/devin" icon={<HostMark host="devin" />} />
  <Card title="Antigravity" description="Google's agentic CLI. Skills via workspace .agents/skills." href="/docs/getting-started/agy" icon={<HostMark host="agy" />} />
</Cards>
