...
Restart Codex after installation. Invoke a workflow explicitly with `$ork-brainstorm`, `$ork-explore`, `$ork-implement`, `$ork-assess`, `$ork-verify`, `$ork-review-pr`, or `$ork-glyph`. Their narrow descriptions also let Codex select the relevant workflow automatically.

Use `$ork-glyph` for a compact inline diagram of status, comparisons, inventories, or pipelines. With no topic supplied, it renders the current conversation.
