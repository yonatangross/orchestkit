...
The product configuration UI is inside Claude Code (and hosts that load the `setup` skill). It is not a page on this site.

- `/ork:setup` runs the full onboarding (scan, stack, safety, **project config**, permissions, skills, MCPs).
- `/ork:setup --configure` skips to Phase 3.5 and writes the env block.

The browser install picker on [Installation](/docs/getting-started/installation) only copies a host install command (for Claude Code, `claude plugin marketplace add …` and `claude plugin install …`). It does not write these knobs.

Writes go to `.claude/settings.json` for a normal project, or `src/settings/ork.settings.json` when you are developing OrchestKit itself.

| Variable | Default | Effect |
|---|---|---|
| `ORCHESTKIT_PROTECTED_BRANCHES` | `main,master` | Blocks direct commits and pushes to these branches |
| `ORCHESTKIT_COMMIT_SCOPE` | `optional` | `optional`, `required`, or `none` |
| `ORCHESTKIT_AGENT_BROWSER_ALLOW_LOCALHOST` | `1` | Allow `*.localhost` in agent-browser (`0` blocks it) |
| `ORCHESTKIT_PERF_SNAPSHOT_ENABLED` | `1` | Write session token snapshots |
| `ORCHESTKIT_LOG_LEVEL` | `warn` | Hook log verbosity (`debug`, `info`, `warn`, `error`) |

Optional webhook: the wizard can save a URL to `.claude/orchestration/config.json`. Events leave the machine only when that URL is set **and** `ORCHESTKIT_HOOK_TOKEN` is exported in the launching shell. Do not put the token in a config file. Skip webhooks unless you run your own collector.

Re-run: `/ork:setup --configure`. Score only: `/ork:setup --score-only`.
