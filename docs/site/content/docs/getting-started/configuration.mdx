...
|---|---|---|
| `MEM0_API_KEY` | Mem0 cloud API key | Enables Tier 3: semantic cross-session memory |
...
| `BRIGHTDATA_API_TOKEN` | BrightData scraping | Enables browser automation MCP for JS-rendered pages |
...
OrchestKit has 4 memory tiers. The first two work automatically:
...
| **3. Mem0** | Cloud API | `MEM0_API_KEY` | Semantic search across all sessions |
| **4. CC Native** | `~/.claude/projects/*/memory/MEMORY.md` | None (auto-sync) | Injected into system prompt |
...
- **[Memory System](/docs/memory/overview)** — Deep dive into 4-tier memory
