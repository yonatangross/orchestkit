...
Slash-command equivalent, typed inside a Claude Code session:
...
| `✗ Hooks: 0 active` | Hook compilation failed | Run `node --version` (must be >= 18). If OK, reinstall: `claude plugin uninstall ork && claude plugin install ork@orchestkit` |
