...
├── ork-lite.json         # Universal toolkit (119 skills)
└── ork.json              # Full specialized (195 skills)
```
...
3. Test: `/plugin marketplace add file://$(pwd)` then `/plugin install ork-lite`
4. Verify: `/ork:doctor`
...
```bash
# For universal skills, add to ork-lite:
vim manifests/ork-lite.json
...
/plugin install ork-lite  # or ork for specialized skills
/ork:doctor
```
