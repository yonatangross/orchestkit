...
```
src/
├── skills/               # 68 skills in flat CC 2.1.7 structure
│   └── <skill-name>/
...
├── agents/               # 37 specialized AI personas
└── hooks/                # 87 TypeScript hooks in 12 split bundles

manifests/                # Plugin definitions (3 JSON files)
├── orkl.json             # Universal toolkit (46 skills)
├── ork-creative.json     # Video production add-on (2 skills)
└── ork.json              # Full specialized (68 skills)
```
...
├── orkl/             # Universal toolkit
└── ork/                  # Full specialized
...
3. Test: `/plugin marketplace add file://$(pwd)` then `/plugin install orkl`
4. Verify: `/ork:doctor`
...
```bash
# For universal skills, add to orkl:
vim manifests/orkl.json

# For specialized skills (Python/React/LLM), add to ork only:
...
/plugin install orkl  # or ork for specialized skills
/ork:doctor
```
